"""
Script to apply independent component analysis (ICA) to CEEMDAN IMF components of XLV (Health Care Select Sector SPDR Fund).

1. Load data required to apply independent component analysis (ICA) to IMF components of XLV.
2. Compute optimal ICA parameters (number of independent components, etc.) for XLV.
2. Apply independent component analysis (ICA) to XLV CEEMDAN IMF components with the computed optimal parameters.
3. Document implemented logic and results in this script.
4. Do not change any existing code for now

Implemented logic
---------------------
- **Load**: ``xlv_imfs_ceemdan.parquet`` (8 oscillatory IMFs + ``Residuo``; the residual is not included in the ICA fit).
...
``xlv_ica_parametros.json`` with full grid.

**Figure:** ``figures/xlv_componentes_ica_ceemdan.png`` (time panel of ``Z_j`` and residual).
"""
...
def cargar_imfs_xlv(ruta_imfs: Path = _RUTA_IMFS_XLV) -> pd.DataFrame:
    ...
    ----------
    ruta_imfs : Path
        Path to ``xlv_imfs_ceemdan.parquet``.
    ...
    if not ruta_imfs.is_file():
        raise FileNotFoundError(
            f"Not found: {ruta_imfs}. Run 03_ceemdan_xlv.py first."
        )
    df = pd.read_parquet(ruta_imfs, engine="pyarrow")
    logger.info("XLV IMFs: %d rows, columns %s", len(df), list(df.columns))
    return df
...
def cargar_indice_temporal_xlv(ruta_xlv: Path = _RUTA_XLV) -> pd.Index:
    """
    Load the date index from ``xlv.parquet`` for the time axis.
    ...
    if not ruta_xlv.is_file():
        raise FileNotFoundError(f"Not found: {ruta_xlv}.")
...
def generar_panel_componentes_ica(
    ...
    fig.suptitle(
        f"XLV: independent FastICA components ({titulo_k}) + CEEMDAN residual",
        fontsize=11,
        y=1.002,
    )
    fig.tight_layout()
...
def guardar_parametros_ica(
    ...
    aplicacion : dict
        Output of :func:`aplicar_ica_xlv`.
    ruta_json : Path
        Output JSON file.
    """
    payload = {
...
def main(
    ...
    """
    Run calibration and FastICA application on XLV.
    ...
    logger.info("ICA k selected for XLV: %d", k_optimo)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="FastICA on CEEMDAN IMFs of XLV with k calibration."
    )
    parser.add_argument(
        "--parquet-imfs",
        type=Path,
        default=None,
        help="IMF parquet (default xlv_imfs_ceemdan.parquet).",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="PNG path (default figures/xlv_componentes_ica_ceemdan.png).",
    )
    ...
    try:
        ...
        logger.exception("Error in ICA for XLV")
        sys.exit(1)
