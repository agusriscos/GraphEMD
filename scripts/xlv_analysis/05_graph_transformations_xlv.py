"""
Script to apply graph transformations to CEEMDAN IMF components of XLV (Health Care Select Sector SPDR Fund).

1. Load data required to apply graph transformations to IMF components of XLV. On one hand, IMF components from the CEEMDAN CEEMDAN decomposition of XLV and on the other hand graph data from graph transformations applied to IMF components of XLV.
2. Apply graph transformations to XLV CEEMDAN IMF components.
3. Document implemented logic and results in this script.
4. Do not change any existing code for now

Implemented logic
---------------------
- **Load**: ``xlv_imfs_ceemdan.parquet`` (8 IMFs + ``Residuo``, 3587 observations; output from
  ``03_ceemdan_xlv.py``).
...
- **Implementation**: ``build_all_imf_graphs`` saves per component parquet + ``.pt``
  (PyTorch Geometric) in ``data/GraphEMD/xlv_analysis/grafos/{hvg,nvg,recurrencia}/``.
- **Auxiliary table**: ``xlv_parametros_recurrencia_ceemdan.csv`` (``tau``, ``d``, ``epsilon`` per IMF).
...
**Recurrence** (``tau``, ``d``, ``epsilon`` in ``xlv_parametros_recurrencia_ceemdan.csv``):
...
**Outputs:** ``grafos/{hvg,nvg,recurrencia}/``, ``xlv_resumen_grafos_imf.csv``,
...
def cargar_imfs_ceemdan_xlv(
    ...
    logger.info(
        "XLV IMFs loaded: %d rows, components %s",
        len(df_imfs),
        columnas,
    )
    return df_imfs
...
def aplicar_transformaciones_grafos(
    ...
    """
    Generate HVG, NVG, and recurrence graphs for each IMF/residual of XLV.
...
def main(
    ...
    """
    Run the IMF → graph transformation pipeline for XLV.

    Parameters
    ----------
    ruta_imfs : Path, optional
        IMF parquet. Default ``xlv_imfs_ceemdan.parquet``.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="IMF→graph transformations (HVG, NVG, recurrence) for XLV CEEMDAN."
    )
    parser.add_argument(
        "--parquet-imfs",
        type=Path,
        default=None,
        help="Parquet with IMF_1,...,Residuo (default xlv_imfs_ceemdan.parquet).",
    )
    parser.add_argument(
        "--dir-grafos",
        type=Path,
    ...
    try:
        ...
        logger.exception("Error in graph transformations for XLV")
        sys.exit(1)
