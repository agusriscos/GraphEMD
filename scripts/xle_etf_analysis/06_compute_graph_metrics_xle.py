"""
Script to obtain graph metrics from graph transformations applied to CEEMDAN IMF components of the XLE.

1. Load data required to obtain graph metrics from graph transformations applied to IMF components of XLE.
2. Obtain density, edge count, connected components, diameter, and mean degree metrics of graphs from graph transformations applied to IMF components of XLE.
3. Compute clustering coefficients (mean, median, and standard deviation) of graphs from graph transformations applied to IMF components of XLE.
4. Compute centralities (degree, betweenness, closeness, and eigenvector) of graphs from graph transformations applied to IMF components of XLE.
5. Document implemented logic and results in this script.
6. Do not change any existing code for now

Implemented logic
---------------------
- **Load**: graphs saved by ``05_graph_transformations_xle.py`` in
...
**Outputs:** ``xle_metricas_grafos_imf.csv``, ``xle_resumen_metricas_grafos.json``.
"""
...
# due to cost on dense graphs (IMF_7+ in XLE exceed ~4×10⁵ edges).
...
def listar_componentes_grafos(dir_grafos: Path = _DIR_GRAFOS) -> list[str]:
    ...
    if not carpeta_hvg.is_dir():
        raise FileNotFoundError(
            f"Not found: {carpeta_hvg}. Run 05_graph_transformations_xle.py first."
        )
...
def main(dir_grafos: Optional[Path] = None) -> dict[str, Any]:
    """
    Entry point: compute and save metrics for all XLE.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Structural, clustering, and centrality metrics for XLE."
    )
    parser.add_argument(
        "--dir-grafos",
        type=Path,
    ...
    try:
        ...
        logger.exception("Error computing graph metrics for XLE")
        sys.exit(1)
