"""
Script to analyze EMD (Empirical Mode Decomposition) preconditions for the XLE (Energy Select Sector SPDR Fund).
...
def cargar_datos_xle(ruta_parquet: Path = _RUTA_PARQUET_XLE) -> pd.DataFrame:
    """
    Load the XLE series and compute daily returns in percent.

    Parameters
    ----------
    ruta_parquet : Path
        Path to the ``xle.parquet``.
...
def main() -> dict[str, Any]:
    """
    Run the full EMD precondition pipeline for XLE.
    ...
    logger.info("=" * 70)
    logger.info("STATIONARITY TESTS — XLE")
    logger.info("=" * 70)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error in EMD condition analysis for XLE")
        sys.exit(1)
