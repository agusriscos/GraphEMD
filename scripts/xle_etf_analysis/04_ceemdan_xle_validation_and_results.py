"""
Script to validate and analyze CEEMDAN decomposition results for XLE (Energy Select Sector SPDR Fund).

1. Load data required to validate and analyze decomposition results for XLE. On one hand, closing price data for XLE and on the other hand IMF component data from the CEEMDAN decomposition of XLE.
2. Obtain statistical information on energy, variance, frequency, and mean amplitude of the IMF components.
3. Validate CEEMDAN decomposition results for XLE by comparing them with MSCI World decomposition results in this repository. Compute reconstruction error of the original series from the IMF components. Assess the decomposition residual for XLE (check whether it represents the long-term trend of the original series (monotonically increasing or decreasing)).
4. Plot IMF components and the original XLE series. Also plot CEEMDAN vs. EEMD decomposition residual for XLE
5. Document implemented logic and results in this script.
6. Do not change any existing code for now

Implemented logic
-------------------
- **Load**: ``xle.parquet``, ``xle_imfs_ceemdan.parquet``; MSCI reference in
...
- **EEMD XLE**: default MSCI parameters (``info_msci_world_data``); figure
...
- **Figures**: ``figures/xle_imfs_panel.png``, ``figures/xle_imf_decomposition_ceemdan_vs_eemd.png``,
  ``figures/xle_tendencia_compuesta_ceemdan.png``.

Results obtained (run 2026-05-16, n=3587)
-------------------------------------------------

**Reconstruction XLE:** ``rmse_relativo≈8.7×10⁻¹⁷`` (numerical identity IMFs+residual).

**IMFs XLE (MSCI-like pattern):** IMF_1 — energy 156, 1282 cycles, amp. 0.15;
IMF_8 — energy 27367, 2 cycles, amp. 2.24; 8 IMFs total.

**Residual XLE:** mean 28.36, range [19.76, 48.28]; not strictly monotonic
...
**Figures:** ``figures/xle_imfs_panel.png``,
``figures/xle_imf_decomposition_ceemdan_vs_eemd.png`` (CEEMDAN vs EEMD gap).

**Outputs:** ``xle_imf_metricas_ceemdan.csv``, ``xle_validacion_ceemdan.json``,
``xle_imfs_eemd.parquet`` (EEMD cache).
"""
...
def cargar_xle() -> tuple[pd.DataFrame, np.ndarray]:
    """
    Load precios XLE prices and the closing series.
    ...
    if not _RUTA_XLE.is_file():
        raise FileNotFoundError(f"Run 01_download_xle.py: missing {_RUTA_XLE}")
...
def cargar_imfs_xle() -> pd.DataFrame:
    ...
    if not _RUTA_IMFS_XLE.is_file():
        raise FileNotFoundError(f"Run 03_ceemdan_xle.py: missing {_RUTA_IMFS_XLE}")
...
def interpretacion_bibliografica_residuo_xle(
    ...
    """
    Write the XLE residual interpretation supported by EMD/CEEMDAN literature.
    ...
    parrafo = (
        f"The CEEMDAN decomposition of XLE is mathematically consistent: the sum of "
...
def generar_figura_tendencia_compuesta(
    ...
    axes[0].set_title(
        f"XLE CEEMDAN: price and composite trend (linear R²={r2_comp:.3f})",
        fontsize=10,
    )
...
def comparar_con_msci(
    ...
    """
    Summarize structural similarities XLE vs. MSCI World.
...
def extract_eemd_imfs_xle(serie: np.ndarray, mod_info: Any) -> pd.DataFrame:
    """
    Obtain EEMD IMFs for XLE (parquet cache if available).
    ...
    if _RUTA_IMFS_EEMD_XLE.is_file():
        logger.info("EEMD XLE loaded from cache: %s", _RUTA_IMFS_EEMD_XLE)
        return pd.read_parquet(_RUTA_IMFS_EEMD_XLE, engine="pyarrow")
    logger.info("Computing EEMD for XLE (may take several minutes)...")
    ...
    logger.info("EEMD XLE saved to %s", _RUTA_IMFS_EEMD_XLE)
    return df_eemd
...
def generar_panel_imfs(
    ...
    axes[-1].set_xlabel("Fecha")
    fig.suptitle("XLE: closing price and CEEMDAN IMFs", fontsize=11, y=1.002)
    fig.tight_layout()
    ruta_salida.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(ruta_salida, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def generar_figura_ceemdan_vs_eemd(
    ...
    """
    ``imf_decomposition.png``-style figure (CEEMDAN vs EEMD) for XLE.
...
def main() -> dict[str, Any]:
    ...
    if len(df_xle) != len(df_imfs_xle):
        raise ValueError(
            f"Different lengths: prices {len(df_xle)} vs IMFs {len(df_imfs_xle)}"
        )

    logger.info("=" * 70)
    logger.info("IMF METRICS — XLE (CEEMDAN)")
    logger.info("=" * 70)
    ...
    logger.info("Reconstruction XLE: %s", validacion_xle)
    logger.info("Residuo XLE: %s", residuo_xle)
    logger.info("XLE composite trend: %s", tendencia_compuesta_xle["expresion"])
    logger.info(
    ...
    df_eemd = extract_eemd_imfs_xle(serie_xle, mod_info)
    generar_panel_imfs(
        df_xle,
        df_imfs_xle,
        serie_xle,
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error in CEEMDAN validation for XLE")
        sys.exit(1)
