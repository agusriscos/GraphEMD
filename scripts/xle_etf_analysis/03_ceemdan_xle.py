"""
Script to decompose XLE (Energy Select Sector SPDR Fund) into IMF components using CEEMDAN.

1. Load XLE data from parquet file data/GraphEMD/xle_etf_analysis/xle.parquet.
...
3. Compute optimal CEEMDAN parameters for XLE using the same logic applied to MSCI World in this repository.
...
Implemented logic
---------------------
- **Load**: ``xle.parquet`` (window 2012-01-12–2026-04-20, 3587 observations aligned to the MSCI calendar).
- **MSCI reference** (``docs/20abr26``, CEEMDAN table): ``max_imf=14``, ``trials=100``,
  ``epsilon=0.05``, ``seed=42``.
- **Calibration XLE** (grid ``epsilon`` × ``trials`` × ``max_imf`` with ``max_imf≥9``):
  in XLE CEEMDAN typically stops at **8 IMFs** even when ``max_imf`` is higher; the criterion
...
- **Final decomposition** with ``calibracion["mejor"]`` (linearity criterion); output in
  ``xle_imfs_ceemdan.parquet`` and ``xle_ceemdan_parametros.json``.

Results obtained
--------------------

After each calibration, review ``xle_ceemdan_parametros.json`` (``mejor`` block) and
re-run ``04_ceemdan_xle_validation_and_results.py`` for figures and validation.
"""
...
def cargar_serie_cierre_xle(ruta_parquet: Path = _RUTA_PARQUET_XLE) -> tuple[pd.DataFrame, np.ndarray]:
    """
    Load XLE and return the closing price vector.
    ...
    if not ruta_parquet.is_file():
        raise FileNotFoundError(
            f"Not found: {ruta_parquet}. Run 01_download_xle.py first."
        )
...
def calibrar_parametros_ceemdan_xle(
    ...
    if mejor_lineal is None:
        raise RuntimeError(
            "CEEMDAN XLE calibration: no configuration meets the RMSE threshold."
        )
    ...
    resumen_n_imfs = {
        ...
        "nota": (
            "CEEMDAN on XLE reaches at most 8 IMFs in this window; "
...
def guardar_parametros_json(
    ...
    ----------
    calibracion : dict
        Output of ``calibrar_parametros_ceemdan_xle``.
    resumen_imfs : dict
...
def main() -> dict[str, Any]:
    """
    Calibrate CEEMDAN on XLE, decompose with optimal parameters, and save outputs.
    ...
    logger.info("=" * 70)
    logger.info(
        "CEEMDAN CALIBRATION — XLE (grid %s×%s×%s, max_imf≥9, linear residual)",
        len(REJILLA_EPSILON),
        len(REJILLA_TRIALS),
        len(REJILLA_MAX_IMF),
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error in CEEMDAN for XLE")
        sys.exit(1)
