"""
Script to apply graph transformations to CEEMDAN IMF components of XLE (Energy Select Sector SPDR Fund).

1. Load data required to apply graph transformations to IMF components of XLE. On one hand, IMF components from the CEEMDAN CEEMDAN decomposition of XLE and on the other hand graph data from graph transformations applied to IMF components of XLE.
2. Apply graph transformations to XLE CEEMDAN IMF components.
3. Document implemented logic and results in this script.
4. Do not change any existing code for now

Implemented logic
---------------------
- **Load**: ``xle_imfs_ceemdan.parquet`` (8 IMFs + ``Residuo``, 3587 observations; output from
  ``03_ceemdan_xle.py``).
...
- **Implementation**: ``build_all_imf_graphs`` saves per component parquet + ``.pt``
  (PyTorch Geometric) in ``data/GraphEMD/xle_etf_analysis/grafos/{hvg,nvg,recurrencia}/``.
- **Auxiliary table**: ``xle_parametros_recurrencia_ceemdan.csv`` (``tau``, ``d``, ``epsilon`` per IMF).
...
**Recurrence** (``tau``, ``d``, ``epsilon`` in ``xle_parametros_recurrencia_ceemdan.csv``):
...
**Outputs:** ``grafos/{hvg,nvg,recurrencia}/``, ``xle_resumen_grafos_imf.csv``,
...
def cargar_imfs_ceemdan_xle(
    ...
    logger.info(
        "XLE IMFs loaded: %d rows, components %s",
        len(df_imfs),
        columnas,
    )
    return df_imfs
...
def aplicar_transformaciones_grafos(
    ...
    """
    Generate HVG, NVG, and recurrence graphs for each IMF/residual of XLE.
...
def main(
    ...
    """
    Run the IMF → graph transformation pipeline for XLE.

    Parameters
    ----------
    ruta_imfs : Path, optional
        IMF parquet. Default ``xle_imfs_ceemdan.parquet``.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="IMF→graph transformations (HVG, NVG, recurrence) for XLE CEEMDAN."
    )
    parser.add_argument(
        "--parquet-imfs",
        type=Path,
        default=None,
        help="Parquet with IMF_1,...,Residuo (default xle_imfs_ceemdan.parquet).",
    )
    parser.add_argument(
        "--dir-grafos",
        type=Path,
    ...
    try:
        ...
        logger.exception("Error in graph transformations for XLE")
        sys.exit(1)
