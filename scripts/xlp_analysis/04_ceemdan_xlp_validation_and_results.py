"""
Script to validate and analyze CEEMDAN decomposition results for XLP (Consumer Staples Select Sector SPDR Fund).

1. Load data required to validate and analyze decomposition results for XLP. On one hand, closing price data for XLP and on the other hand IMF component data from the CEEMDAN decomposition of XLP.
2. Obtain statistical information on energy, variance, frequency, and mean amplitude of the IMF components.
3. Validate CEEMDAN decomposition results for XLP by comparing them with MSCI World decomposition results in this repository. Compute reconstruction error of the original series from the IMF components. Assess the decomposition residual for XLP (check whether it represents the long-term trend of the original series (monotonically increasing or decreasing)).
4. Plot IMF components and the original XLP. Tambien pinta el residuo de la descomposicion CEEMDAN vs. EEMD para XLP
5. Document implemented logic and results in this script.
6. Do not change any existing code for now

Implemented logic
-------------------
- **Load**: ``xlp.parquet``, ``xlp_imfs_ceemdan.parquet``; MSCI reference in
...
- **EEMD XLP**: default MSCI parameters (``info_msci_world_data``); figure
...
- **Figures**: ``figures/xlp_imfs_panel.png``, ``figures/xlp_imf_decomposition_ceemdan_vs_eemd.png``,
  ``figures/xlp_tendencia_compuesta_ceemdan.png``.

Results obtained (run 2026-05-16, n=3587)
-------------------------------------------------

**Reconstruction XLP:** ``rmse_relativo≈8.7×10⁻¹⁷`` (numerical identity IMFs+residual).

**IMFs XLP (MSCI-like pattern):** IMF_1 — energy 156, 1282 cycles, amp. 0.15;
IMF_8 — energy 27367, 2 cycles, amp. 2.24; 8 IMFs total.

**Residual XLP:** mean 28.36, range [19.76, 48.28]; not strictly monotonic
...
**Figures:** ``figures/xlp_imfs_panel.png``,
``figures/xlp_imf_decomposition_ceemdan_vs_eemd.png`` (CEEMDAN vs EEMD gap).

**Outputs:** ``xlp_imf_metricas_ceemdan.csv``, ``xlp_validacion_ceemdan.json``,
``xlp_imfs_eemd.parquet`` (EEMD cache).
"""
...
def cargar_xlp() -> tuple[pd.DataFrame, np.ndarray]:
    """
    Load precios XLP prices and the closing series.
    ...
    if not _RUTA_XLP.is_file():
        raise FileNotFoundError(f"Run 01_download_xlp.py: missing {_RUTA_XLP}")
...
def cargar_imfs_xlp() -> pd.DataFrame:
    ...
    if not _RUTA_IMFS_XLP.is_file():
        raise FileNotFoundError(f"Run 03_ceemdan_xlp.py: missing {_RUTA_IMFS_XLP}")
...
def interpretacion_bibliografica_residuo_xlp(
    ...
    """
    Write the XLP residual interpretation supported by EMD/CEEMDAN literature.
    ...
    parrafo = (
        f"The CEEMDAN decomposition of XLP is mathematically consistent: the sum of "
...
def generar_figura_tendencia_compuesta(
    ...
    axes[0].set_title(
        f"XLP CEEMDAN: price and composite trend (linear R²={r2_comp:.3f})",
        fontsize=10,
    )
...
def comparar_con_msci(
    ...
    """
    Summarize structural similarities XLP vs. MSCI World.
...
def extract_eemd_imfs_xlp(serie: np.ndarray, mod_info: Any) -> pd.DataFrame:
    """
    Obtain EEMD IMFs for XLP (parquet cache if available).
    ...
    if _RUTA_IMFS_EEMD_XLP.is_file():
        logger.info("EEMD XLP loaded from cache: %s", _RUTA_IMFS_EEMD_XLP)
        return pd.read_parquet(_RUTA_IMFS_EEMD_XLP, engine="pyarrow")
    logger.info("Computing EEMD for XLP (may take several minutes)...")
    ...
    logger.info("EEMD XLP saved to %s", _RUTA_IMFS_EEMD_XLP)
    return df_eemd
...
def generar_panel_imfs(
    ...
    axes[-1].set_xlabel("Fecha")
    fig.suptitle("XLP: closing price and CEEMDAN IMFs", fontsize=11, y=1.002)
    fig.tight_layout()
    ruta_salida.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(ruta_salida, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def generar_figura_ceemdan_vs_eemd(
    ...
    """
    ``imf_decomposition.png``-style figure (CEEMDAN vs EEMD) for XLP.
...
def main() -> dict[str, Any]:
    ...
    if len(df_xlp) != len(df_imfs_xlp):
        raise ValueError(
            f"Different lengths: prices {len(df_xlp)} vs IMFs {len(df_imfs_xlp)}"
        )

    logger.info("=" * 70)
    logger.info("IMF METRICS — XLP (CEEMDAN)")
    logger.info("=" * 70)
    ...
    logger.info("Reconstruction XLP: %s", validacion_xlp)
    logger.info("Residuo XLP: %s", residuo_xlp)
    logger.info("XLP composite trend: %s", tendencia_compuesta_xlp["expresion"])
    logger.info(
    ...
    df_eemd = extract_eemd_imfs_xlp(serie_xlp, mod_info)
    generar_panel_imfs(
        df_xlp,
        df_imfs_xlp,
        serie_xlp,
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error in CEEMDAN validation for XLP")
        sys.exit(1)
