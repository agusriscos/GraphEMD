"""
Script to download XLP (Consumer Staples Select Sector SPDR Fund) data from Yahoo Finance.

Implemented logic
---------------------
- Download history of ``XLP`` with ``yfinance`` (``period="max"``).
- Trim to inclusive dates ``2012-01-12`` … ``2026-04-20`` (same window as ``docs/20abr26``).
- If ``data/20abr26/msci_world.parquet``, align the index of XLP to MSCI World
...
def descargar_xlp_yahoo() -> pd.DataFrame:
    """
    Download the full historical series for XLP data from Yahoo Finance.
    ...
    """
    logger.info("Descargando %s data from Yahoo Finance...", SIMBOLO_XLP)
    ticker = yf.Ticker(SIMBOLO_XLP)
    df = ticker.history(period="max")
    if df is None or df.empty:
        raise ValueError(f"Could not download data for {SIMBOLO_XLP}.")
    logger.info(
...
def alinear_a_calendario_msci(
    ...
    """
    Reindexa XLP to the date calendar of ``msci_world.parquet``.

    Parameters
    ----------
    df_xlp : pd.DataFrame
        XLP already trimmed to the time window.
    ruta_msci : Path
    ...
    -------
    pd.DataFrame
        XLP with the same index as MSCI (only dates present in both).
    ...
    if faltantes > 0:
        logger.warning(
            "%s MSCI dates without XLP quote; those rows are dropped.",
            faltantes,
        )
...
def main() -> dict[str, Any]:
    """
    Download XLP, align to the MSCI window, save parquet, and compute metrics.
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error downloading or analyzing XLP")
        sys.exit(1)
