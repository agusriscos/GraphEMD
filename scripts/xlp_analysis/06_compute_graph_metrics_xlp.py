"""
Script to obtain graph metrics from graph transformations applied to CEEMDAN IMF components of the XLP.

1. Load data required to obtain graph metrics from graph transformations applied to IMF components of XLP.
2. Obtain density, edge count, connected components, diameter, and mean degree metrics of graphs from graph transformations applied to IMF components of XLP.
3. Compute clustering coefficients (mean, median, and standard deviation) of graphs from graph transformations applied to IMF components of XLP.
4. Compute centralities (degree, betweenness, closeness, and eigenvector) of graphs from graph transformations applied to IMF components of XLP.
5. Document implemented logic and results in this script.
6. Do not change any existing code for now

Implemented logic
---------------------
- **Load**: graphs saved by ``05_graph_transformations_xlp.py`` in
...
**Outputs:** ``xlp_metricas_grafos_imf.csv``, ``xlp_resumen_metricas_grafos.json``.
"""
...
# due to cost on dense graphs (IMF_7+ in XLP exceed ~4×10⁵ edges).
...
def listar_componentes_grafos(dir_grafos: Path = _DIR_GRAFOS) -> list[str]:
    ...
    if not carpeta_hvg.is_dir():
        raise FileNotFoundError(
            f"Not found: {carpeta_hvg}. Run 05_graph_transformations_xlp.py first."
        )
...
def main(dir_grafos: Optional[Path] = None) -> dict[str, Any]:
    """
    Entry point: compute and save metrics for all XLP.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Structural, clustering, and centrality metrics for XLP."
    )
    parser.add_argument(
        "--dir-grafos",
        type=Path,
    ...
    try:
        ...
        logger.exception("Error computing graph metrics for XLP")
        sys.exit(1)
