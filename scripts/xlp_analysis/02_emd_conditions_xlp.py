"""
Script to analyze EMD (Empirical Mode Decomposition) preconditions for the XLP (Consumer Staples Select Sector SPDR Fund).
...
def cargar_datos_xlp(ruta_parquet: Path = _RUTA_PARQUET_XLP) -> pd.DataFrame:
    """
    Load the XLP series and compute daily returns in percent.

    Parameters
    ----------
    ruta_parquet : Path
        Path to the ``xlp.parquet``.
...
def main() -> dict[str, Any]:
    """
    Run the full EMD precondition pipeline for XLP.
    ...
    logger.info("=" * 70)
    logger.info("STATIONARITY TESTS — XLP")
    logger.info("=" * 70)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error in EMD condition analysis for XLP")
        sys.exit(1)
