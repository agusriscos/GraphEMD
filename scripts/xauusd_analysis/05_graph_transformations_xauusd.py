"""
Script to apply graph transformations to CEEMDAN IMF components of XAU/USD (spot gold vs. US dollar).

1. Load data required to apply graph transformations to IMF components of XAU/USD. On one hand, IMF components from the CEEMDAN CEEMDAN decomposition of XAU/USD and on the other hand graph data from graph transformations applied to IMF components of XAU/USD.
2. Apply graph transformations to XAU/USD CEEMDAN IMF components.
3. Document implemented logic and results in this script.
4. Do not change any existing code for now

Implemented logic
---------------------
- **Load**: ``xauusd_imfs_ceemdan.parquet`` (8 IMFs + ``Residuo``, 3587 observations; output from
  ``03_ceemdan_xauusd.py``).
...
- **Implementation**: ``build_all_imf_graphs`` saves per component parquet + ``.pt``
  (PyTorch Geometric) in ``data/GraphEMD/xauusd_analysis/grafos/{hvg,nvg,recurrencia}/``.
- **Auxiliary table**: ``xauusd_parametros_recurrencia_ceemdan.csv`` (``tau``, ``d``, ``epsilon`` per IMF).
...
**Recurrence** (``tau``, ``d``, ``epsilon`` in ``xauusd_parametros_recurrencia_ceemdan.csv``):
...
**Outputs:** ``grafos/{hvg,nvg,recurrencia}/``, ``xauusd_resumen_grafos_imf.csv``,
``xauusd_parametros_recurrencia_ceemdan.csv``, ``xauusd_grafos_imf_manifest.json``.
"""
...
_RUTA_RESUMEN_CSV = _DIR_DATOS / "xauusd_resumen_grafos_imf.csv"
_RUTA_PARAM_RECURRENCIA_CSV = _DIR_DATOS / "xauusd_parametros_recurrencia_ceemdan.csv"
_RUTA_RESUMEN_JSON = _DIR_DATOS / "xauusd_grafos_imf_manifest.json"
...
def cargar_imfs_ceemdan_xauusd(
    ruta_imfs: Path = _RUTA_IMFS_XAUUSD,
) -> pd.DataFrame:
    """
    ...
    ----------
    ruta_imfs : Path
        Path to ``xauusd_imfs_ceemdan.parquet``.
    ...
    if not ruta_imfs.is_file():
        raise FileNotFoundError(
            f"Not found: {ruta_imfs}. Run 03_ceemdan_xauusd.py first."
        )
    ...
    logger.info(
        "XAU/USD IMFs loaded: %d rows, components %s",
        len(df_imfs),
        columnas,
    )
    return df_imfs
...
def aplicar_transformaciones_grafos(
    ...
    """
    Generate HVG, NVG, and recurrence graphs for each IMF/residual of XAU/USD.
...
def guardar_manifest(
    ...
    """
    payload = {
        "ruta_imfs": str(_RUTA_IMFS_XAUUSD),
...
def main(
    ...
    """
    Run the IMF → graph transformation pipeline for XAU/USD.

    Parameters
    ----------
    ruta_imfs : Path, optional
        IMF parquet. Default ``xauusd_imfs_ceemdan.parquet``.
    ...
    df_imfs = cargar_imfs_ceemdan_xauusd(ruta)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="IMF→graph transformations (HVG, NVG, recurrence) for XAU/USD CEEMDAN."
    )
    parser.add_argument(
        "--parquet-imfs",
        type=Path,
        default=None,
        help="Parquet with IMF_1,...,Residuo (default xauusd_imfs_ceemdan.parquet).",
    )
    parser.add_argument(
        "--dir-grafos",
        type=Path,
    ...
    try:
        ...
        logger.exception("Error in graph transformations for XAU/USD")
        sys.exit(1)
