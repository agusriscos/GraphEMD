"""
Script to analyze EMD (Empirical Mode Decomposition) preconditions for XAU/USD (spot gold vs. US dollar).
...
def cargar_datos_xauusd(ruta_parquet: Path = _RUTA_PARQUET_XAUUSD) -> pd.DataFrame:
    """
    Load the XAU/USD series and compute daily returns in percent.

    Parameters
    ----------
    ruta_parquet : Path
        Path to the ``xauusd.parquet``.
...
def main() -> dict[str, Any]:
    """
    Run the full EMD precondition pipeline for XAU/USD.
    ...
    logger.info("=" * 70)
    logger.info("STATIONARITY TESTS — XAU/USD")
    logger.info("=" * 70)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error in EMD condition analysis for XAU/USD")
        sys.exit(1)
