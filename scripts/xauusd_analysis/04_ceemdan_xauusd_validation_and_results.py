"""
Script to validate and analyze CEEMDAN decomposition results for XAU/USD (spot gold vs. US dollar).

1. Load data required to validate and analyze decomposition results for XAU/USD. On one hand, closing price data for XAU/USD and on the other hand IMF component data from the CEEMDAN decomposition of XAU/USD.
2. Obtain statistical information on energy, variance, frequency, and mean amplitude of the IMF components.
3. Validate CEEMDAN decomposition results for XAU/USD by comparing them with MSCI World decomposition results in this repository. Compute reconstruction error of the original series from the IMF components. Assess the decomposition residual for XAU/USD (check whether it represents the long-term trend of the original series (monotonically increasing or decreasing)).
4. Plot IMF components and the original XAU/USD series. Also plot CEEMDAN vs. EEMD decomposition residual for XAU/USD
5. Document implemented logic and results in this script.
6. Do not change any existing code for now

Implemented logic
-------------------
- **Load**: ``xauusd.parquet``, ``xauusd_imfs_ceemdan.parquet``; MSCI reference in
...
- **EEMD XAU/USD**: default MSCI parameters (``info_msci_world_data``); figure
...
- **Figures**: ``figures/xauusd_imfs_panel.png``, ``figures/xauusd_imf_decomposition_ceemdan_vs_eemd.png``,
  ``figures/xauusd_tendencia_compuesta_ceemdan.png``.

Results obtained (run 2026-05-16, n=3587)
-------------------------------------------------

**Reconstruction XAU/USD:** ``rmse_relativo≈8.7×10⁻¹⁷`` (numerical identity IMFs+residual).

**IMFs XAU/USD (MSCI-like pattern):** IMF_1 — energy 156, 1282 cycles, amp. 0.15;
IMF_8 — energy 27367, 2 cycles, amp. 2.24; 8 IMFs total.

**Residual XAU/USD:** mean 28.36, range [19.76, 48.28]; not strictly monotonic
...
**Figures:** ``figures/xauusd_imfs_panel.png``,
``figures/xauusd_imf_decomposition_ceemdan_vs_eemd.png`` (CEEMDAN vs EEMD gap).

**Outputs:** ``xauusd_imf_metricas_ceemdan.csv``, ``xauusd_validacion_ceemdan.json``,
``xauusd_imfs_eemd.parquet`` (EEMD cache).
"""
...
def cargar_xauusd() -> tuple[pd.DataFrame, np.ndarray]:
    """
    Load precios XAU/USD prices and the closing series.
    ...
    if not _RUTA_XAUUSD.is_file():
        raise FileNotFoundError(f"Run 01_download_xauusd.py: missing {_RUTA_XAUUSD}")
...
def cargar_imfs_xauusd() -> pd.DataFrame:
    """
    Load the CEEMDAN IMF parquet for XAU/USD.
    ...
    if not _RUTA_IMFS_XAUUSD.is_file():
        raise FileNotFoundError(f"Run 03_ceemdan_xauusd.py: missing {_RUTA_IMFS_XAUUSD}")
...
def interpretacion_bibliografica_residuo_xauusd(
    ...
    """
    Write the XAU/USD residual interpretation supported by EMD/CEEMDAN literature.
    ...
    parrafo = (
        f"The CEEMDAN decomposition of XAU/USD is mathematically consistent: the sum of "
...
def generar_figura_tendencia_compuesta(
    ...
    axes[0].set_title(
        f"XAU/USD CEEMDAN: price and composite trend (linear R²={r2_comp:.3f})",
        fontsize=10,
    )
...
def comparar_con_msci(
    ...
    """
    Summarize structural similarities XAU/USD vs. MSCI World.
...
def extract_eemd_imfs_xauusd(serie: np.ndarray, mod_info: Any) -> pd.DataFrame:
    """
    Obtain EEMD IMFs for XAU/USD (parquet cache if available).
    ...
    if _RUTA_IMFS_EEMD_XAUUSD.is_file():
        logger.info("EEMD XAU/USD loaded from cache: %s", _RUTA_IMFS_EEMD_XAUUSD)
        return pd.read_parquet(_RUTA_IMFS_EEMD_XAUUSD, engine="pyarrow")
    logger.info("Computing EEMD for XAU/USD (may take several minutes)...")
    ...
    logger.info("EEMD XAU/USD saved to %s", _RUTA_IMFS_EEMD_XAUUSD)
    return df_eemd
...
def generar_panel_imfs(
    ...
    axes[-1].set_xlabel("Fecha")
    fig.suptitle("XAU/USD: closing price and CEEMDAN IMFs", fontsize=11, y=1.002)
    fig.tight_layout()
    ruta_salida.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(ruta_salida, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def generar_figura_ceemdan_vs_eemd(
    ...
    """
    ``imf_decomposition.png``-style figure (CEEMDAN vs EEMD) for XAU/USD.
...
def main() -> dict[str, Any]:
    ...
    if len(df_xauusd) != len(df_imfs_xauusd):
        raise ValueError(
            f"Different lengths: prices {len(df_xauusd)} vs IMFs {len(df_imfs_xauusd)}"
        )

    logger.info("=" * 70)
    logger.info("IMF METRICS — XAU/USD (CEEMDAN)")
    logger.info("=" * 70)
    ...
    logger.info("Reconstruction XAU/USD: %s", validacion_xauusd)
    logger.info("Residuo XAU/USD: %s", residuo_xauusd)
    logger.info("XAU/USD composite trend: %s", tendencia_compuesta_xauusd["expresion"])
    logger.info(
    ...
    df_eemd = extract_eemd_imfs_xauusd(serie_xauusd, mod_info)
    generar_panel_imfs(
        df_xauusd,
        df_imfs_xauusd,
        serie_xauusd,
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error in CEEMDAN validation for XAU/USD")
        sys.exit(1)
