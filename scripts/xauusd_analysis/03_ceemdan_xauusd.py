"""
Script to decompose XAU/USD (spot gold vs. US dollar) into IMF components using CEEMDAN.

1. Load XAU/USD data from parquet file data/GraphEMD/xauusd_analysis/xauusd.parquet.
...
3. Compute optimal CEEMDAN parameters for XAU/USD using the same logic applied to MSCI World in this repository.
...
Implemented logic
---------------------
- **Load**: ``xauusd.parquet`` (window 2012-01-12–2026-04-20, 3587 observations aligned to the MSCI calendar).
- **MSCI reference** (``docs/20abr26``, CEEMDAN table): ``max_imf=14``, ``trials=100``,
  ``epsilon=0.05``, ``seed=42``.
- **Calibration XAU/USD** (grid ``epsilon`` × ``trials`` × ``max_imf`` with ``max_imf≥9``):
  in XAU/USD, CEEMDAN typically stops at **8 IMFs** even when ``max_imf`` is higher; the criterion
...
- **Final decomposition** with ``calibracion["mejor"]`` (linearity criterion); output in
  ``xauusd_imfs_ceemdan.parquet`` and ``xauusd_ceemdan_parametros.json``.

Results obtained
--------------------

After each calibration, review ``xauusd_ceemdan_parametros.json`` (``mejor`` block) and
re-run ``04_ceemdan_xauusd_validation_and_results.py`` for figures and validation.
"""
...
def cargar_serie_cierre_xauusd(ruta_parquet: Path = _RUTA_PARQUET_XAUUSD) -> tuple[pd.DataFrame, np.ndarray]:
    """
    Load XAU/USD and return the closing price vector.

    Parameters
    ----------
    ruta_parquet : Path
        Path to ``xauusd.parquet``.
    ...
    if not ruta_parquet.is_file():
        raise FileNotFoundError(
            f"Not found: {ruta_parquet}. Run 01_download_xauusd.py first."
        )
...
def calibrar_parametros_ceemdan_xauusd(
    ...
    if mejor_lineal is None:
        raise RuntimeError(
            "CEEMDAN XAU/USD calibration: no configuration meets the RMSE threshold."
        )
    ...
    resumen_n_imfs = {
        ...
        "nota": (
            "CEEMDAN on XAU/USD reaches at most 8 IMFs in this window; "
...
def guardar_parametros_json(
    ...
    ----------
    calibracion : dict
        Output of ``calibrar_parametros_ceemdan_xauusd``.
    resumen_imfs : dict
...
def main() -> dict[str, Any]:
    """
    Calibrate CEEMDAN on XAU/USD, decompose with optimal parameters, and save outputs.
    ...
    logger.info("=" * 70)
    logger.info(
        "CEEMDAN CALIBRATION — XAU/USD (grid %s×%s×%s, max_imf≥9, linear residual)",
        len(REJILLA_EPSILON),
        len(REJILLA_TRIALS),
        len(REJILLA_MAX_IMF),
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error in CEEMDAN for XAU/USD")
        sys.exit(1)
