"""
Script to download XAU/USD (spot XAU/USD gold vs. US dollar) data from Yahoo Finance.

Implemented logic
---------------------
- Download COMEX futures ``GC=F`` (gold in USD/oz) with ``yfinance`` (``period="max"``).
- Trim to inclusive dates ``2012-01-12`` … ``2026-04-20`` (same window as ``docs/20abr26``).
- If ``data/20abr26/msci_world.parquet``, align the index of XAU/USD to MSCI World
...
# COMEX gold/USD futures (Yahoo does not reliably expose XAUUSD=X).
...
def _ruta_script() -> Path:
    ...
    -------
    Path
        Path to the ``download_xauusd.py``.
    """
...
def descargar_xauusd_yahoo() -> pd.DataFrame:
    """
    Download the full historical series for XAU/USD data from Yahoo Finance.
    ...
    """
    logger.info("Descargando %s data from Yahoo Finance...", SIMBOLO_XAUUSD)
    ticker = yf.Ticker(SIMBOLO_XAUUSD)
    df = ticker.history(period="max")
    if df is None or df.empty:
        raise ValueError(f"Could not download data for {SIMBOLO_XAUUSD}.")
    logger.info(
...
def alinear_a_calendario_msci(
    ...
    """
    Reindexa XAU/USD to the date calendar of ``msci_world.parquet``.

    Parameters
    ----------
    df_xauusd : pd.DataFrame
        XAU/USD already trimmed to the time window.
    ruta_msci : Path
    ...
    -------
    pd.DataFrame
        XAU/USD with the same index as MSCI (only dates present in both).
    ...
    if faltantes > 0:
        logger.warning(
            "%s MSCI dates without XAU/USD quote; those rows are dropped.",
            faltantes,
        )
...
def main() -> dict[str, Any]:
    """
    Download XAU/USD, align to the MSCI window, save parquet, and compute metrics.
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error downloading or analyzing XAU/USD")
        sys.exit(1)
