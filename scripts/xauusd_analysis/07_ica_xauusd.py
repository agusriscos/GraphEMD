"""
Script to apply independent component analysis (ICA) to CEEMDAN IMF components of XAU/USD (spot gold vs. US dollar).

1. Load data required to apply independent component analysis (ICA) to IMF components of XAU/USD.
2. Compute optimal ICA parameters (number of independent components, etc.) for XAU/USD.
2. Apply independent component analysis (ICA) to XAU/USD CEEMDAN IMF components with the computed optimal parameters.
3. Document implemented logic and results in this script.
4. Do not change any existing code for now

Implemented logic
---------------------
- **Load**: ``xauusd_imfs_ceemdan.parquet`` (8 oscillatory IMFs + ``Residuo``; the residual is not included in the ICA fit).
...
**Calibration:** grid ``k=2…p−1``; automatic selection by ``R²`` elbow + 90% threshold +
composite score (see ``detalle_seleccion`` in JSON).

**Applied output:** ``Z_1…Z_4`` + ``Residuo`` in ``ica/fastica/imfs_reducidas.parquet``;
``xauusd_ica_parametros.json`` with full grid.

**Figure:** ``figures/xauusd_componentes_ica_ceemdan.png`` (time panel of ``Z_j`` and residual).
"""
...
def cargar_imfs_xauusd(ruta_imfs: Path = _RUTA_IMFS_XAUUSD) -> pd.DataFrame:
    ...
    logger.info("XAU/USD IMFs: %d rows, columns %s", len(df), list(df.columns))
    return df
...
def cargar_indice_temporal_xauusd(ruta_xauusd: Path = _RUTA_XAUUSD) -> pd.Index:
    """
    Load the date index from ``xauusd.parquet`` for the time axis.

    Parameters
    ----------
    ruta_xauusd : Path
        price parquet XAU/USD.
    ...
    if not ruta_xauusd.is_file():
        raise FileNotFoundError(f"Not found: {ruta_xauusd}.")
...
def generar_panel_componentes_ica(
    ...
    fig.suptitle(
        f"XAU/USD: independent FastICA components ({titulo_k}) + CEEMDAN residual",
        fontsize=11,
        y=1.002,
    )
    fig.tight_layout()
...
def generar_figura_desde_parquet(
    ...
    ruta_xauusd : Path
        prices XAU/USD for the time index.
    ruta_salida : Path
        Output PNG.
    k : int, optional
...
def guardar_parametros_ica(
    ...
    aplicacion : dict
        Output of :func:`aplicar_ica_xauusd`.
    ruta_json : Path
        Output JSON file.
    """
    payload = {
...
def main(
    ...
    """
    Run calibration and FastICA application on XAU/USD.
    ...
    logger.info("ICA k selected for XAU/USD: %d", k_optimo)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="FastICA on CEEMDAN IMFs of XAU/USD with k calibration."
    )
    parser.add_argument(
        "--parquet-imfs",
        type=Path,
        default=None,
        help="IMF parquet (default xauusd_imfs_ceemdan.parquet).",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="PNG path (default figures/xauusd_componentes_ica_ceemdan.png).",
    )
    ...
    try:
        ...
        logger.exception("Error in ICA for XAU/USD")
        sys.exit(1)
