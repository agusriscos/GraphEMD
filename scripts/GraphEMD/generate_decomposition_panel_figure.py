#!/usr/bin/env python3
"""
Genera una figura con cinco paneles CEEMDAN/EEMD/VMD del panel empírico en
disposición 3 filas × 2 columnas; el quinto activo queda centrado en la fila inferior.

Cada celda replica el panel de ``generar_figura_comparativa_residuos`` en
``ejecutar_vmd_todos_activos.py`` (señal + suma de IMFs arriba; brecha de residuo abajo).
Sin título global ni leyenda (convención de color en el pie de figura LaTeX).

Ejemplo::

    python scripts/GraphEMD/generar_figura_descomposicion_panel_activos.py
...
def _cargar_modulo_vmd_activos() -> Any:
    """
    Carga ``ejecutar_vmd_todos_activos`` para reutilizar rutas y utilidades IMF.

    Returns
    -------
    module
        Módulo con ``ACTIVOS``, ``cargar_serie_precios`` y helpers de brecha/residuo.
    """
...
def _leer_vmd_dc(ruta_parametros: Path) -> int:
    ...
    -------
    int
        Valor de ``DC`` (por defecto 1 si falta el archivo).
    """
    if not ruta_parametros.is_file():
        logger.warning("Sin %s; se asume DC=1.", ruta_parametros)
        return 1
...
def _dibujar_panel_activo(
    ...
    """
    Dibuja el par superior/inferior CEEMDAN/EEMD/VMD para un activo.

    Parameters
    ----------
    mod : module
        Módulo ``ejecutar_vmd_todos_activos``.
    cfg : ConfigActivo
        Configuración del activo.
    ax_superior : matplotlib.axes.Axes
        Eje de la señal original y sumas de IMFs.
    ax_inferior : matplotlib.axes.Axes
        Eje de brechas de tendencia/residuo implícito.
    """
    ...
    if not rutas["imfs_ceemdan"].is_file():
        raise FileNotFoundError(f"Falta CEEMDAN: {rutas['imfs_ceemdan']}")
    if not rutas["imfs"].is_file():
        raise FileNotFoundError(f"Falta VMD: {rutas['imfs']}")
...
def generar_figura_msci_ilustrativa(
    ...
    """
    Figura ilustrativa comparativa CEEMDAN/EEMD/VMD en MSCI World.

    Panel superior: señal original y sumas de IMFs oscilatorios por método.
    Panel inferior: brecha de tendencia lenta (residuo implícito) por método.
    ...
    ruta_salida_pdf : pathlib.Path, optional
        Si se indica, también guarda PDF vectorial.
    dpi : int
        Resolución del PNG.
    ...
    plt.close(fig)
    logger.info("Figura ilustrativa MSCI: %s", ruta_salida_png)
...
def generar_figura(
    ...
    """
    Construye la figura 3×2 con cinco activos (dos filas internas por celda).

    La cuadrícula usa cuatro columnas lógicas: filas 1–2 ocupan pares de columnas;
    la fila 3 centra el quinto activo en las columnas intermedias.
    ...
    plt.close(fig)
    logger.info("Figura panel activos: %s", ruta_salida_png)
    return ruta_salida_png
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Figura CEEMDAN/EEMD/VMD para los cinco activos del panel."
    )
    parser.add_argument(
        "--salida-png",
        type=Path,
    ...
    parser.add_argument(
        "--copia-docs",
        action="store_true",
        help="También escribe bajo docs/20abr26/images/english/.",
    )
    parser.add_argument("--dpi", type=int, default=200, help="Resolución del PNG.")
    parser.add_argument(
        "--msci-ilustrativa",
        action="store_true",
        help="Genera solo la figura ilustrativa MSCI (reconstrucción + residuo CEEMDAN).",
    )
...
def main() -> None:
    """
    Punto de entrada: genera la figura del panel empírico.
    """
