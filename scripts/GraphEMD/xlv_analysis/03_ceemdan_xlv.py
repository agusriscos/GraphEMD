"""
Script para descomponer la ETF XLV (Health Care Select Sector SPDR Fund) en componentes IMFs mediante CEEMDAN.

1. Carga los datos de la ETF XLV desde el archivo parquet data/GraphEMD/xlv_analysis/xlv.parquet.
...
3. Calcula los parametros optimos de CEEMDAN para el XLV usando la misma logica que se ha realizado para el MSCI World en este mismo repositorio.
...
Lógica implementada
---------------------
- **Carga**: ``xlv.parquet`` (ventana 2012-01-12–2026-04-20, 3587 observaciones alineadas al calendario MSCI).
- **Referencia MSCI** (``docs/20abr26``, tabla CEEMDAN): ``max_imf=14``, ``trials=100``,
  ``epsilon=0.05``, ``seed=42``.
- **Calibración XLV** (rejilla ``epsilon`` × ``trials`` × ``max_imf`` con ``max_imf≥9``):
  en XLV CEEMDAN suele detenerse en **8 IMFs** aunque ``max_imf`` sea mayor; el criterio
...
- **Descomposición final** con ``calibracion["mejor"]`` (criterio linealidad); salida en
  ``xlv_imfs_ceemdan.parquet`` y ``xlv_ceemdan_parametros.json``.

Resultados obtenidos
--------------------

Tras cada calibración, revisar ``xlv_ceemdan_parametros.json`` (bloque ``mejor``) y
re-ejecutar ``04_ceemdan_xlv_validation_and_results.py`` para figuras y validación.
"""
...
_RUTA_PARQUET_XLV = _DIR_DATOS / "xlv.parquet"
_RUTA_IMFS_SALIDA = _DIR_DATOS / "xlv_imfs_ceemdan.parquet"
_RUTA_PARAMETROS_JSON = _DIR_DATOS / "xlv_ceemdan_parametros.json"
...
def _cargar_modulo_emdsynth_pipeline():
    ...
    spec = importlib.util.spec_from_file_location("emdsynth_ejecutar_xlv", ruta)
...
def cargar_serie_cierre_xlv(ruta_parquet: Path = _RUTA_PARQUET_XLV) -> tuple[pd.DataFrame, np.ndarray]:
    """
    Carga XLV y devuelve el vector de precios de cierre.
    ...
    if not ruta_parquet.is_file():
        raise FileNotFoundError(
            f"No se encontró {ruta_parquet}. Ejecute antes 01_download_xlv.py."
        )
...
def calibrar_parametros_ceemdan_xlv(
    serie: np.ndarray,
    mod_emdsynth: Any,
    ...
    if mejor_lineal is None:
        raise RuntimeError(
            "Calibración CEEMDAN XLV: ninguna configuración cumple el umbral de RMSE."
        )
    ...
    resumen_n_imfs = {
        ...
        "nota": (
            "CEEMDAN en XLV alcanza como máximo 8 IMFs en esta ventana; "
...
def guardar_parametros_json(
    ...
    ----------
    calibracion : dict
        Salida de ``calibrar_parametros_ceemdan_xlv``.
    resumen_imfs : dict
...
def main() -> dict[str, Any]:
    """
    Calibra CEEMDAN en XLV, descompone con parámetros óptimos y guarda salidas.
    ...
    df, serie = cargar_serie_cierre_xlv()

    logger.info("=" * 70)
    logger.info(
        "CALIBRACIÓN CEEMDAN — XLV (rejilla %s×%s×%s, max_imf≥9, residuo lineal)",
        len(REJILLA_EPSILON),
        len(REJILLA_TRIALS),
        len(REJILLA_MAX_IMF),
    )
    logger.info("=" * 70)
    calibracion = calibrar_parametros_ceemdan_xlv(serie, mod)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error en CEEMDAN de XLV")
        sys.exit(1)
