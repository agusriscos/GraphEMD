"""
Script para descargar los datos de la ETF XLV (Health Care Select Sector SPDR Fund) desde Yahoo Finance.

Lógica implementada
---------------------
- Descarga histórico de ``XLV`` con ``yfinance`` (``period="max"``).
- Recorte por fechas inclusivas ``2012-01-12`` … ``2026-04-20`` (misma ventana que ``docs/20abr26``).
- Si existe ``data/20abr26/msci_world.parquet``, se alinea el índice de XLV al de MSCI World
...
- Parquet: ``data/GraphEMD/xlv_analysis/xlv.parquet``
"""
...
SIMBOLO_XLV: str = "XLV"
NOMBRE_ARCHIVO_PARQUET: str = "xlv.parquet"
...
def _ruta_script() -> Path:
    ...
    -------
    Path
        Ruta al archivo ``download_xlv.py``.
    """
    return Path(__file__).resolve()


def descargar_xlv_yahoo() -> pd.DataFrame:
    """
    Descarga el histórico completo de XLV desde Yahoo Finance.
    ...
    """
    logger.info("Descargando %s desde Yahoo Finance...", SIMBOLO_XLV)
    ticker = yf.Ticker(SIMBOLO_XLV)
    df = ticker.history(period="max")
    if df is None or df.empty:
        raise ValueError(f"No se pudieron descargar datos para {SIMBOLO_XLV}.")
    logger.info(
...
def alinear_a_calendario_msci(
    ...
    """
    Reindexa XLV al calendario de fechas de ``msci_world.parquet``.

    Parameters
    ----------
    df_xlv : pd.DataFrame
        XLV ya recortado a la ventana temporal.
    ruta_msci : Path
    ...
    -------
    pd.DataFrame
        XLV con el mismo índice que MSCI (solo fechas presentes en ambos).
    ...
    xlv_norm = _normalizar_indice_fechas(df_xlv)
    alineado = xlv_norm.reindex(fechas_msci)
    faltantes = alineado["Close"].isna().sum()
    if faltantes > 0:
        logger.warning(
            "%s fechas MSCI sin cotización XLV; se eliminan esas filas.",
            faltantes,
        )
...
def main() -> dict[str, Any]:
    """
    Descarga XLV, alinea a la ventana MSCI, guarda parquet y calcula métricas.
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error en la descarga o el análisis de XLV")
        sys.exit(1)
