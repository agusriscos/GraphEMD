"""
Script para aplicar el analisis de componentes independientes (ICA) a los componentes IMFs de la ETF XLV (Health Care Select Sector SPDR Fund) mediante CEEMDAN.

1. Carga los datos necesarios para la aplicacion del analisis de componentes independientes (ICA) a los componentes IMFs de la ETF XLV.
2. Calcula los parametros optimos de ICA (numero de componentes independientes, etc) XLV.
2. Aplica el analisis de componentes independientes (ICA) a los componentes IMFs de la ETF XLV mediante CEEMDAN con los parametros optimos calculados.
...
Lógica implementada
---------------------
- **Carga**: ``xlv_imfs_ceemdan.parquet`` (8 IMF oscilatorias + ``Residuo``; el residuo no entra en el ajuste ICA).
...
**Calibración:** rejilla ``k=2…7``; selección automática por rodilla ``R²`` + umbral 90 % +
score compuesto (ver ``detalle_seleccion`` en JSON).

**Salida aplicada:** ``Z_1…Z_4`` + ``Residuo`` en ``ica/fastica/imfs_reducidas.parquet``;
``xlv_ica_parametros.json`` con rejilla completa.

**Figura:** ``figures/xlv_componentes_ica_ceemdan.png`` (panel temporal de ``Z_j`` y residuo).
"""
...
_RUTA_PARAMETROS_JSON = _DIR_DATOS / "xlv_ica_parametros.json"
_RUTA_XLV = _DIR_DATOS / "xlv.parquet"
_DIR_FIGURAS = _DIR_DATOS / "figures"
_RUTA_FIGURA_ICA = _DIR_FIGURAS / "xlv_componentes_ica_ceemdan.png"
...
def cargar_imfs_xlv(ruta_imfs: Path = _RUTA_IMFS_XLV) -> pd.DataFrame:
    """
    ...
    ----------
    ruta_imfs : Path
        Ruta a ``xlv_imfs_ceemdan.parquet``.
    ...
    if not ruta_imfs.is_file():
        raise FileNotFoundError(
            f"No se encuentra {ruta_imfs}. Ejecute antes 03_ceemdan_xlv.py."
        )
    df = pd.read_parquet(ruta_imfs, engine="pyarrow")
    logger.info("IMFs XLV: %d filas, columnas %s", len(df), list(df.columns))
    return df
...
def cargar_indice_temporal_xlv(ruta_xlv: Path = _RUTA_XLV) -> pd.Index:
    """
    Carga el índice de fechas de ``xlv.parquet`` para el eje temporal.

    Parameters
    ----------
    ruta_xlv : Path
        Parquet de precios XLV.
    ...
    FileNotFoundError
        Si no existe el archivo.
    """
    if not ruta_xlv.is_file():
        raise FileNotFoundError(f"No se encuentra {ruta_xlv}.")
    df = pd.read_parquet(ruta_xlv, engine="pyarrow")
...
def generar_panel_componentes_ica(
    ...
    fig.suptitle(
        f"XLV: componentes independientes FastICA ({titulo_k}) + residuo CEEMDAN",
        fontsize=11,
        y=1.002,
    )
    fig.tight_layout()
...
def generar_figura_desde_parquet(
    ...
    ruta_xlv : Path
        Precios XLV para el índice temporal.
    ruta_salida : Path
        PNG de salida.
    k : int, optional
    ...
    indice = cargar_indice_temporal_xlv(ruta_xlv)
    if k is None:
...
def guardar_parametros_ica(
    ...
    aplicacion : dict
        Salida de :func:`aplicar_ica_xlv`.
    ruta_json : Path
        Archivo JSON de salida.
    """
    payload = {
        "parquet_entrada": str(_RUTA_IMFS_XLV.resolve()),
...
def main(
    ...
    """
    Ejecuta calibración y aplicación de FastICA sobre las IMF XLV.
    ...
    """
    ruta = ruta_imfs or _RUTA_IMFS_XLV
    salida = dir_salida or _DIR_SALIDA_ICA

    df = cargar_imfs_xlv(ruta)
    ...
    logger.info("k seleccionado para ICA XLV: %d", k_optimo)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="FastICA sobre IMF CEEMDAN de XLV con calibración de k."
    )
    parser.add_argument(
        "--parquet-imfs",
        type=Path,
        default=None,
        help="Parquet IMF (por defecto xlv_imfs_ceemdan.parquet).",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Ruta del PNG (por defecto figures/xlv_componentes_ica_ceemdan.png).",
    )
    ...
    try:
        ...
        logger.exception("Error en ICA XLV")
        sys.exit(1)
