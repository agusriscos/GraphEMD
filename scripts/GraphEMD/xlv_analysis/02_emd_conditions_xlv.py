"""
Script para analizar las condiciones de la ETF XLV (Health Care Select Sector SPDR Fund) para su analisis con EMD (Empirical Mode Decomposition).
...
def cargar_datos_xlv(ruta_parquet: Path = _RUTA_PARQUET_XLV) -> pd.DataFrame:
    """
    Carga la serie XLV y calcula retornos diarios en porcentaje.

    Parameters
    ----------
    ruta_parquet : Path
        Ruta al archivo ``xlv.parquet``.
...
def main() -> dict[str, Any]:
    """
    Ejecuta el pipeline completo de condiciones EMD para XLV.
    ...
    logger.info("=" * 70)
    logger.info("TESTS DE ESTACIONARIDAD — XLV")
    logger.info("=" * 70)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error en el análisis de condiciones EMD de XLV")
        sys.exit(1)
