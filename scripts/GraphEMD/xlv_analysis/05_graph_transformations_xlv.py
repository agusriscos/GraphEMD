"""
Script para aplicar las transformaciones de grafos a los componentes IMFs de la ETF XLV (Health Care Select Sector SPDR Fund) mediante CEEMDAN.

1. Carga los datos necesarios para la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLV. Por un lado los datos de los componentes IMFs obtenidos en la descomposición de la ETF XLV mediante CEEMDAN y por otro lado los datos de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLV.
2. Aplica las transformaciones de grafos a los componentes IMFs de la ETF XLV mediante CEEMDAN.
...
Lógica implementada
---------------------
- **Carga**: ``xlv_imfs_ceemdan.parquet`` (8 IMFs + ``Residuo``, 3587 observaciones; salida de
  ``03_ceemdan_xlv.py``).
...
- **Implementación**: ``obtener_grafos_all_imf`` guarda por componente parquet + ``.pt``
  (PyTorch Geometric) en ``data/GraphEMD/xlv_analysis/grafos/{hvg,nvg,recurrencia}/``.
- **Tabla auxiliar**: ``xlv_parametros_recurrencia_ceemdan.csv`` (``tau``, ``d``, ``epsilon`` por IMF).
...
**Recurrencia** (``tau``, ``d``, ``epsilon`` en ``xlv_parametros_recurrencia_ceemdan.csv``):
...
**Salidas:** ``grafos/{hvg,nvg,recurrencia}/``, ``xlv_resumen_grafos_imf.csv``,
``xlv_parametros_recurrencia_ceemdan.csv``, ``xlv_grafos_imf_manifest.json``.
"""
...
_RUTA_RESUMEN_CSV = _DIR_DATOS / "xlv_resumen_grafos_imf.csv"
_RUTA_PARAM_RECURRENCIA_CSV = _DIR_DATOS / "xlv_parametros_recurrencia_ceemdan.csv"
_RUTA_RESUMEN_JSON = _DIR_DATOS / "xlv_grafos_imf_manifest.json"
...
def cargar_imfs_ceemdan_xlv(
    ruta_imfs: Path = _RUTA_IMFS_XLV,
) -> pd.DataFrame:
    """
    ...
    logger.info(
        "IMFs XLV cargadas: %d filas, componentes %s",
        len(df_imfs),
        columnas,
    )
    return df_imfs
...
def aplicar_transformaciones_grafos(
    ...
    """
    Genera HVG, NVG y grafos de recurrencia para cada IMF/residuo de XLV.
...
def main(
    ...
    """
    Ejecuta la pipeline de transformación IMF → grafos para XLV.

    Parameters
    ----------
    ruta_imfs : Path, optional
        Parquet de IMFs. Por defecto ``xlv_imfs_ceemdan.parquet``.
    ...
    df_imfs = cargar_imfs_ceemdan_xlv(ruta)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Transformaciones IMF→grafo (HVG, NVG, recurrencia) para XLV CEEMDAN."
    )
    parser.add_argument(
        "--parquet-imfs",
        type=Path,
        default=None,
        help="Parquet con IMF_1,...,Residuo (por defecto xlv_imfs_ceemdan.parquet).",
    )
    parser.add_argument(
        "--dir-grafos",
        type=Path,
    ...
    try:
        ...
        logger.exception("Error en transformaciones de grafos XLV")
        sys.exit(1)
