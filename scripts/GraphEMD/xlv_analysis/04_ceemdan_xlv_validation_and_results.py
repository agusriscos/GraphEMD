"""
Script para validar y analizar los resultados de la descomposición de la ETF XLV (Health Care Select Sector SPDR Fund) en componentes IMFs mediante CEEMDAN.

1. Carga los datos necesarios para la validacion y analisis de los resultados de la descomposición de la ETF XLV. Por un lado los datos de precio de cierre de la ETF XLV y por otro lado los datos de los componentes IMFs obtenidos en la descomposición de la ETF XLV mediante CEEMDAN.
2. Obten la información estadistica sobre energía, variancia, frecuencia y amplitud media de los componentes IMFs obtenidos.
3. Valida los resultados de la descomposición de la ETF XLV mediante CEEMDAN comparandolos con los resultados de la descomposición de la MSCI World en este mismo repositorio. Calcula el error de la reconstrucción de la serie original con los componentes IMFs obtenidos. Evalua como es el residuo de la descomposicion de la ETF XLV (mira si representa la tendencia a largo plazo de la serie original (monotona creciente o decreciente)).
4. Pinta las graficas de los componentes IMFs obtenidos y la serie original de la ETF XLV. Tambien pinta el residuo de la descomposicion CEEMDAN vs. EEMD para el XLV
...
Lógica implementada
-------------------
- **Carga**: ``xlv.parquet``, ``xlv_imfs_ceemdan.parquet``; referencia MSCI en
...
- **EEMD XLV**: parámetros por defecto MSCI (``info_msci_world_data``); figura
...
- **Figuras**: ``figures/xlv_imfs_panel.png``, ``figures/xlv_imf_decomposition_ceemdan_vs_eemd.png``,
  ``figures/xlv_tendencia_compuesta_ceemdan.png``.

Resultados obtenidos (ejecución 2026-05-16, n=3587)
-------------------------------------------------

**Reconstrucción XLV:** ``rmse_relativo≈8.7×10⁻¹⁷`` (identidad numérica IMFs+residuo).

**IMFs XLV (patrón tipo MSCI):** IMF_1 — energía 156, 1282 ciclos, amp. 0.15;
IMF_8 — energía 27367, 2 ciclos, amp. 2.24; 8 IMFs en total.

**Residuo XLV:** media 28.36, rango [19.76, 48.28]; no estrictamente monótono
...
**Figuras:** ``figures/xlv_imfs_panel.png``,
``figures/xlv_imf_decomposition_ceemdan_vs_eemd.png`` (brecha CEEMDAN vs EEMD).

**Salidas:** ``xlv_imf_metricas_ceemdan.csv``, ``xlv_validacion_ceemdan.json``,
``xlv_imfs_eemd.parquet`` (caché EEMD).
"""
...
def cargar_xlv() -> tuple[pd.DataFrame, np.ndarray]:
    """
    Carga precios XLV y la serie de cierre.
    ...
    if not _RUTA_XLV.is_file():
        raise FileNotFoundError(f"Ejecute 01_download_xlv.py: falta {_RUTA_XLV}")
...
def cargar_imfs_xlv() -> pd.DataFrame:
    """
    Carga el parquet de IMFs CEEMDAN de XLV.
    ...
    if not _RUTA_IMFS_XLV.is_file():
        raise FileNotFoundError(f"Ejecute 03_ceemdan_xlv.py: falta {_RUTA_IMFS_XLV}")
...
def interpretacion_bibliografica_residuo_xlv(
    ...
    """
    Redacta la interpretación del residuo XLV apoyada en literatura EMD/CEEMDAN.
    ...
    parrafo = (
        f"La descomposición CEEMDAN de XLV es matemáticamente consistente: la suma de "
...
def generar_figura_tendencia_compuesta(
    ...
    axes[0].set_title(
        f"XLV CEEMDAN: precio y tendencia compuesta (R² lineal={r2_comp:.3f})",
        fontsize=10,
    )
...
def comparar_con_msci(
    ...
    """
    Resume similitudes estructurales XLV vs MSCI World.
...
def obtener_imfs_eemd_xlv(serie: np.ndarray, mod_info: Any) -> pd.DataFrame:
    """
    Obtiene IMFs EEMD de XLV (cache en parquet si existe).
    ...
    if _RUTA_IMFS_EEMD_XLV.is_file():
        logger.info("EEMD XLV cargado desde caché: %s", _RUTA_IMFS_EEMD_XLV)
        return pd.read_parquet(_RUTA_IMFS_EEMD_XLV, engine="pyarrow")
    logger.info("Calculando EEMD para XLV (puede tardar varios minutos)...")
    ...
    logger.info("EEMD XLV guardado en %s", _RUTA_IMFS_EEMD_XLV)
    return df_eemd
...
def generar_panel_imfs(
    ...
    axes[-1].set_xlabel("Fecha")
    fig.suptitle("XLV: precio de cierre e IMFs CEEMDAN", fontsize=11, y=1.002)
    fig.tight_layout()
    ruta_salida.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(ruta_salida, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def generar_figura_ceemdan_vs_eemd(
    ...
    """
    Figura estilo ``imf_decomposition.png`` (CEEMDAN vs EEMD) para XLV.
...
def main() -> dict[str, Any]:
    ...
    if len(df_xlv) != len(df_imfs_xlv):
        raise ValueError(
            f"Longitudes distintas: precios {len(df_xlv)} vs IMFs {len(df_imfs_xlv)}"
        )

    logger.info("=" * 70)
    logger.info("MÉTRICAS IMF — XLV (CEEMDAN)")
    logger.info("=" * 70)
    ...
    logger.info("Reconstrucción XLV: %s", validacion_xlv)
    logger.info("Residuo XLV: %s", residuo_xlv)
    logger.info("Tendencia compuesta XLV: %s", tendencia_compuesta_xlv["expresion"])
    logger.info(
    ...
    df_eemd = obtener_imfs_eemd_xlv(serie_xlv, mod_info)
    generar_panel_imfs(
        df_xlv,
        df_imfs_xlv,
        serie_xlv,
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error en validación CEEMDAN de XLV")
        sys.exit(1)
