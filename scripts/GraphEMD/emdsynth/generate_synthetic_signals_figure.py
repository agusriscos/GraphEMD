#!/usr/bin/env python3
"""
Genera una figura PDF/PNG con las cinco señales sintéticas usadas en el artículo
(disposición 5 filas por 1 columna, sin título global; el pie de figura va en LaTeX).

Los parámetros coinciden con ``construir_escenarios`` en
``ejecutar_descomposiciones_emdsynth.py``.

Ejemplo::

    python3 scripts/GraphEMD/emdsynth/generar_figura_senales_sinteticas.py
"""
...
def _cargar_modulo_ejecutar():
    """
    Carga el módulo del pipeline de descomposiciones (misma definición de escenarios).

    Returns
    -------
    module
        Módulo con ``construir_escenarios``.
    """
...
def generar_figura(
    ...
    """
    Construye la figura con cinco subplots apilados (una fila por señal).
...
def main() -> None:
    """
    Punto de entrada: escribe PNG (y PDF) bajo ``docs/20abr26/images/english/``.
    """
