"""
Script para obtener las metricas de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLP.

1. Carga los datos necesarios para la obtencion de las metricas de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLP.
2. Obten las metricas de densidad, numero de enlaces, componentes conexas, diametro y grado medio de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLP.
3. Calcula los coeficientes de clustering (media, mediana y desviacion estandar) de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLP.
4. Calcula las centralidades (degree, betweenness, closeness y eigenvector) de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLP.
...
Lógica implementada
---------------------
- **Carga**: grafos guardados por ``05_graph_transformations_xlp.py`` en
  ``data/GraphEMD/xlp_analysis/grafos/{hvg,nvg,recurrencia}/{imf_k,residuo}/*.pt``
...
**Salidas:** ``xlp_metricas_grafos_imf.csv``, ``xlp_resumen_metricas_grafos.json``.
"""
...
_RUTA_METRICAS_CSV = _DIR_DATOS / "xlp_metricas_grafos_imf.csv"
_RUTA_RESUMEN_JSON = _DIR_DATOS / "xlp_resumen_metricas_grafos.json"

TIPOS_GRAFO: tuple[str, ...] = ("hvg", "nvg", "recurrencia")

# Umbrales alineados con scripts MSCI (exploracion/); clustering NVG acotado
# por coste en grafos densos (IMF_7+ en XLP superan ~4×10⁵ aristas).
...
def listar_componentes_grafos(dir_grafos: Path = _DIR_GRAFOS) -> list[str]:
    ...
    if not carpeta_hvg.is_dir():
        raise FileNotFoundError(
            f"No existe {carpeta_hvg}. Ejecute antes 05_graph_transformations_xlp.py."
        )
...
def main(dir_grafos: Optional[Path] = None) -> dict[str, Any]:
    """
    Punto de entrada: calcula y guarda métricas de todos los grafos XLP.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Métricas estructurales, clustering y centralidad de grafos IMF XLP."
    )
    parser.add_argument(
        "--dir-grafos",
        type=Path,
    ...
    try:
        ...
        logger.exception("Error al calcular métricas de grafos XLP")
        sys.exit(1)
