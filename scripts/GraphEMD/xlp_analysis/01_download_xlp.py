"""
Script para descargar los datos de la ETF XLP (Consumer Staples Select Sector SPDR Fund) desde Yahoo Finance.

Lógica implementada
---------------------
- Descarga histórico de ``XLP`` con ``yfinance`` (``period="max"``).
- Recorte por fechas inclusivas ``2012-01-12`` … ``2026-04-20`` (misma ventana que ``docs/20abr26``).
- Si existe ``data/20abr26/msci_world.parquet``, se alinea el índice de XLP al de MSCI World
...
- Parquet: ``data/GraphEMD/xlp_analysis/xlp.parquet``
"""
...
SIMBOLO_XLP: str = "XLP"
NOMBRE_ARCHIVO_PARQUET: str = "xlp.parquet"
...
def _ruta_script() -> Path:
    ...
    -------
    Path
        Ruta al archivo ``download_xlp.py``.
    """
    return Path(__file__).resolve()


def descargar_xlp_yahoo() -> pd.DataFrame:
    """
    Descarga el histórico completo de XLP desde Yahoo Finance.
    ...
    """
    logger.info("Descargando %s desde Yahoo Finance...", SIMBOLO_XLP)
    ticker = yf.Ticker(SIMBOLO_XLP)
    df = ticker.history(period="max")
    if df is None or df.empty:
        raise ValueError(f"No se pudieron descargar datos para {SIMBOLO_XLP}.")
    logger.info(
...
def alinear_a_calendario_msci(
    ...
    """
    Reindexa XLP al calendario de fechas de ``msci_world.parquet``.

    Parameters
    ----------
    df_xlp : pd.DataFrame
        XLP ya recortado a la ventana temporal.
    ruta_msci : Path
    ...
    -------
    pd.DataFrame
        XLP con el mismo índice que MSCI (solo fechas presentes en ambos).
    ...
    xlp_norm = _normalizar_indice_fechas(df_xlp)
    alineado = xlp_norm.reindex(fechas_msci)
    faltantes = alineado["Close"].isna().sum()
    if faltantes > 0:
        logger.warning(
            "%s fechas MSCI sin cotización XLP; se eliminan esas filas.",
            faltantes,
        )
...
def main() -> dict[str, Any]:
    """
    Descarga XLP, alinea a la ventana MSCI, guarda parquet y calcula métricas.
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error en la descarga o el análisis de XLP")
        sys.exit(1)
