"""
Script para analizar las condiciones de la ETF XLP (Consumer Staples Select Sector SPDR Fund) para su analisis con EMD (Empirical Mode Decomposition).
...
_RUTA_PARQUET_XLP = _REPO_ROOT / "data" / "GraphEMD" / "xlp_analysis" / "xlp.parquet"
...
def cargar_datos_xlp(ruta_parquet: Path = _RUTA_PARQUET_XLP) -> pd.DataFrame:
    """
    Carga la serie XLP y calcula retornos diarios en porcentaje.

    Parameters
    ----------
    ruta_parquet : Path
        Ruta al archivo ``xlp.parquet``.
...
def main() -> dict[str, Any]:
    """
    Ejecuta el pipeline completo de condiciones EMD para XLP.
    ...
    logger.info("=" * 70)
    logger.info("TESTS DE ESTACIONARIDAD — XLP")
    logger.info("=" * 70)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error en el análisis de condiciones EMD de XLP")
        sys.exit(1)
