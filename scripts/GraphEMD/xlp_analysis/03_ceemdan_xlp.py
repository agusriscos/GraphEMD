"""
Script para descomponer la ETF XLP (Consumer Staples Select Sector SPDR Fund) en componentes IMFs mediante CEEMDAN.

1. Carga los datos de la ETF XLP desde el archivo parquet data/GraphEMD/xlp_analysis/xlp.parquet.
...
3. Calcula los parametros optimos de CEEMDAN para XLP usando la misma logica que se ha realizado para el MSCI World en este mismo repositorio.
...
Lógica implementada
---------------------
- **Carga**: ``xlp.parquet`` (ventana 2012-01-12–2026-04-20, 3587 observaciones alineadas al calendario MSCI).
- **Referencia MSCI** (``docs/20abr26``, tabla CEEMDAN): ``max_imf=14``, ``trials=100``,
  ``epsilon=0.05``, ``seed=42``.
- **Calibración XLP** (rejilla ``epsilon`` × ``trials`` × ``max_imf`` con ``max_imf≥9``):
  en XLP CEEMDAN suele detenerse en **8 IMFs** aunque ``max_imf`` sea mayor; el criterio
...
- **Descomposición final** con ``calibracion["mejor"]`` (criterio linealidad); salida en
  ``xlp_imfs_ceemdan.parquet`` y ``xlp_ceemdan_parametros.json``.

Resultados obtenidos
--------------------

Tras cada calibración, revisar ``xlp_ceemdan_parametros.json`` (bloque ``mejor``) y
re-ejecutar ``04_ceemdan_xlp_validation_and_results.py`` para figuras y validación.
"""
...
def cargar_serie_cierre_xlp(ruta_parquet: Path = _RUTA_PARQUET_XLP) -> tuple[pd.DataFrame, np.ndarray]:
    """
    Carga XLP y devuelve el vector de precios de cierre.
    ...
    if not ruta_parquet.is_file():
        raise FileNotFoundError(
            f"No se encontró {ruta_parquet}. Ejecute antes 01_download_xlp.py."
        )
...
def calibrar_parametros_ceemdan_xlp(
    ...
    if mejor_lineal is None:
        raise RuntimeError(
            "Calibración CEEMDAN XLP: ninguna configuración cumple el umbral de RMSE."
        )
    ...
    resumen_n_imfs = {
        ...
        "nota": (
            "CEEMDAN en XLP alcanza como máximo 8 IMFs en esta ventana; "
...
def guardar_parametros_json(
    ...
    ----------
    calibracion : dict
        Salida de ``calibrar_parametros_ceemdan_xlp``.
    resumen_imfs : dict
...
def main() -> dict[str, Any]:
    """
    Calibra CEEMDAN en XLP, descompone con parámetros óptimos y guarda salidas.
    ...
    logger.info("=" * 70)
    logger.info(
        "CALIBRACIÓN CEEMDAN — XLP (rejilla %s×%s×%s, max_imf≥9, residuo lineal)",
        len(REJILLA_EPSILON),
        len(REJILLA_TRIALS),
        len(REJILLA_MAX_IMF),
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error en CEEMDAN de XLP")
        sys.exit(1)
