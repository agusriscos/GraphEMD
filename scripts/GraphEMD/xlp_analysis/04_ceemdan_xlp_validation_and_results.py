"""
Script para validar y analizar los resultados de la descomposición de la ETF XLP (Consumer Staples Select Sector SPDR Fund) en componentes IMFs mediante CEEMDAN.

1. Carga los datos necesarios para la validacion y analisis de los resultados de la descomposición de la ETF XLP. Por un lado los datos de precio de cierre de la ETF XLP y por otro lado los datos de los componentes IMFs obtenidos en la descomposición de la ETF XLP mediante CEEMDAN.
2. Obten la información estadistica sobre energía, variancia, frecuencia y amplitud media de los componentes IMFs obtenidos.
3. Valida los resultados de la descomposición de la ETF XLP mediante CEEMDAN comparandolos con los resultados de la descomposición de la MSCI World en este mismo repositorio. Calcula el error de la reconstrucción de la serie original con los componentes IMFs obtenidos. Evalua como es el residuo de la descomposicion de la ETF XLP (mira si representa la tendencia a largo plazo de la serie original (monotona creciente o decreciente)).
4. Pinta las graficas de los componentes IMFs obtenidos y la serie original de la ETF XLP. Tambien pinta el residuo de la descomposicion CEEMDAN vs. EEMD para XLP
...
Lógica implementada
-------------------
- **Carga**: ``xlp.parquet``, ``xlp_imfs_ceemdan.parquet``; referencia MSCI en
...
- **EEMD XLP**: parámetros por defecto MSCI (``info_msci_world_data``); figura
...
- **Figuras**: ``figures/xlp_imfs_panel.png``, ``figures/xlp_imf_decomposition_ceemdan_vs_eemd.png``,
  ``figures/xlp_tendencia_compuesta_ceemdan.png``.

Resultados obtenidos (ejecución 2026-05-16, n=3587)
-------------------------------------------------

**Reconstrucción XLP:** ``rmse_relativo≈8.7×10⁻¹⁷`` (identidad numérica IMFs+residuo).

**IMFs XLP (patrón tipo MSCI):** IMF_1 — energía 156, 1282 ciclos, amp. 0.15;
IMF_8 — energía 27367, 2 ciclos, amp. 2.24; 8 IMFs en total.

**Residuo XLP:** media 28.36, rango [19.76, 48.28]; no estrictamente monótono
...
**Figuras:** ``figures/xlp_imfs_panel.png``,
``figures/xlp_imf_decomposition_ceemdan_vs_eemd.png`` (brecha CEEMDAN vs EEMD).

**Salidas:** ``xlp_imf_metricas_ceemdan.csv``, ``xlp_validacion_ceemdan.json``,
``xlp_imfs_eemd.parquet`` (caché EEMD).
"""
...
_RUTA_IMFS_EEMD_XLP = _DIR_DATOS / "xlp_imfs_eemd.parquet"
...
_RUTA_METRICAS_CSV = _DIR_DATOS / "xlp_imf_metricas_ceemdan.csv"
_RUTA_VALIDACION_JSON = _DIR_DATOS / "xlp_validacion_ceemdan.json"
_RUTA_FIGURA_TENDENCIA_COMPUESTA = (
    _DIR_FIGURAS / "xlp_tendencia_compuesta_ceemdan.png"
)
...
def _cargar_modulo_info_msci() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("info_msci_world_data_xlp", ruta)
...
def cargar_xlp() -> tuple[pd.DataFrame, np.ndarray]:
    """
    Carga precios XLP y la serie de cierre.
    ...
    """
    if not _RUTA_XLP.is_file():
        raise FileNotFoundError(f"Ejecute 01_download_xlp.py: falta {_RUTA_XLP}")
    df = pd.read_parquet(_RUTA_XLP, engine="pyarrow")
    serie = np.asarray(df["Close"].values, dtype=np.float64)
    return df, serie


def cargar_imfs_xlp() -> pd.DataFrame:
    """
    Carga el parquet de IMFs CEEMDAN de XLP.
    ...
    """
    if not _RUTA_IMFS_XLP.is_file():
        raise FileNotFoundError(f"Ejecute 03_ceemdan_xlp.py: falta {_RUTA_IMFS_XLP}")
    return pd.read_parquet(_RUTA_IMFS_XLP, engine="pyarrow")
...
def interpretacion_bibliografica_residuo_xlp(
    validacion_reconstruccion: dict[str, Any],
    residuo_eval: dict[str, Any],
    tendencia_compuesta_eval: dict[str, Any],
) -> dict[str, Any]:
    """
    Redacta la interpretación del residuo XLP apoyada en literatura EMD/CEEMDAN.
    ...
    parrafo = (
        f"La descomposición CEEMDAN de XLP es matemáticamente consistente: la suma de "
...
def generar_figura_tendencia_compuesta(
    ...
    axes[0].plot(fechas, serie, color="0.35", linewidth=0.7, label="Close XLP")
    axes[0].plot(
        fechas,
        tendencia,
        color="C1",
    ...
    axes[0].set_title(
        f"XLP CEEMDAN: precio y tendencia compuesta (R² lineal={r2_comp:.3f})",
        fontsize=10,
    )
...
def comparar_con_msci(
    tabla_xlp: pd.DataFrame,
    tabla_msci: Optional[pd.DataFrame],
    validacion_xlp: dict[str, Any],
    validacion_msci: Optional[dict[str, Any]],
) -> dict[str, Any]:
    """
    Resume similitudes estructurales XLP vs MSCI World.
    ...
    """
    out: dict[str, Any] = {
        "n_imfs_xlp": len(tabla_xlp),
        "validacion_xlp": validacion_xlp,
    }
    if tabla_msci is not None:
        out["n_imfs_msci"] = len(tabla_msci)
        out["validacion_msci"] = validacion_msci
        out["frecuencia_imf1_xlp"] = float(tabla_xlp.loc["IMF_1", "frecuencia_ciclos"])
        out["frecuencia_imf1_msci"] = float(tabla_msci.loc["IMF_1", "frecuencia_ciclos"])
        out["amplitud_imf8_xlp"] = float(
            tabla_xlp.loc[tabla_xlp.index[-1], "amplitud_media"]
        )
...
def obtener_imfs_eemd_xlp(serie: np.ndarray, mod_info: Any) -> pd.DataFrame:
    """
    Obtiene IMFs EEMD de XLP (cache en parquet si existe).
    ...
    """
    if _RUTA_IMFS_EEMD_XLP.is_file():
        logger.info("EEMD XLP cargado desde caché: %s", _RUTA_IMFS_EEMD_XLP)
        return pd.read_parquet(_RUTA_IMFS_EEMD_XLP, engine="pyarrow")
    logger.info("Calculando EEMD para XLP (puede tardar varios minutos)...")
    ...
    df_eemd.to_parquet(_RUTA_IMFS_EEMD_XLP, engine="pyarrow", index=False)
    logger.info("EEMD XLP guardado en %s", _RUTA_IMFS_EEMD_XLP)
    return df_eemd
...
def generar_panel_imfs(
    ...
    axes[-1].set_xlabel("Fecha")
    fig.suptitle("XLP: precio de cierre e IMFs CEEMDAN", fontsize=11, y=1.002)
    fig.tight_layout()
    ruta_salida.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(ruta_salida, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def generar_figura_ceemdan_vs_eemd(
    ...
    """
    Figura estilo ``imf_decomposition.png`` (CEEMDAN vs EEMD) para XLP.
    ...
    destino = directorio / "xlp_imf_decomposition_ceemdan_vs_eemd.png"
    if origen.is_file():
...
def main() -> dict[str, Any]:
    ...
    df_xlp, serie_xlp = cargar_xlp()
    df_imfs_xlp = cargar_imfs_xlp()
    if len(df_xlp) != len(df_imfs_xlp):
        raise ValueError(
            f"Longitudes distintas: precios {len(df_xlp)} vs IMFs {len(df_imfs_xlp)}"
        )

    logger.info("=" * 70)
    logger.info("MÉTRICAS IMF — XLP (CEEMDAN)")
    logger.info("=" * 70)
    tabla_xlp = tabla_metricas_imfs(df_imfs_xlp)
    logger.info("\n%s", tabla_xlp.to_string(float_format=lambda x: f"{x:.4f}"))

    validacion_xlp = validar_reconstruccion(serie_xlp, df_imfs_xlp)
    residuo_xlp = evaluar_residuo(serie_xlp, df_imfs_xlp)
    tendencia_compuesta_xlp = evaluar_tendencia_compuesta(
        serie_xlp, df_imfs_xlp, residuo_xlp
    )
    interpretacion_xlp = interpretacion_bibliografica_residuo_xlp(
        validacion_xlp,
        residuo_xlp,
        tendencia_compuesta_xlp,
    )
    logger.info("Reconstrucción XLP: %s", validacion_xlp)
    logger.info("Residuo XLP: %s", residuo_xlp)
    logger.info("Tendencia compuesta XLP: %s", tendencia_compuesta_xlp["expresion"])
    logger.info(
        "R² residuo=%.3f, R² compuesta=%.3f, corr(Close) residuo=%.3f, compuesta=%.3f",
        tendencia_compuesta_xlp["metricas_residuo"]["r2_regresion_lineal"],
        tendencia_compuesta_xlp["metricas_tendencia_compuesta"]["r2_regresion_lineal"],
        tendencia_compuesta_xlp["metricas_residuo"]["correlacion_con_close"],
        tendencia_compuesta_xlp["correlacion_tendencia_compuesta_con_close"],
    )
    ...
    comparacion = comparar_con_msci(
        tabla_xlp, tabla_msci, validacion_xlp, validacion_msci
    )

    _DIR_DATOS.mkdir(parents=True, exist_ok=True)
    tabla_xlp.to_csv(_RUTA_METRICAS_CSV)
    payload = {
        "n_observaciones": len(serie_xlp),
        "metricas_imf_xlp": tabla_xlp.reset_index().to_dict(orient="records"),
        "validacion_reconstruccion_xlp": validacion_xlp,
        "evaluacion_residuo_xlp": residuo_xlp,
        "evaluacion_tendencia_compuesta_xlp": tendencia_compuesta_xlp,
        "interpretacion_residuo_ceemdan_xlp": interpretacion_xlp,
    ...
    df_eemd = obtener_imfs_eemd_xlp(serie_xlp, mod_info)
    generar_panel_imfs(
        df_xlp,
        df_imfs_xlp,
        serie_xlp,
        _DIR_FIGURAS / "xlp_imfs_panel.png",
    )
    ...
    return {
        "tabla_xlp": tabla_xlp,
        "validacion_xlp": validacion_xlp,
        "residuo_xlp": residuo_xlp,
        "tendencia_compuesta_xlp": tendencia_compuesta_xlp,
        "interpretacion_xlp": interpretacion_xlp,
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error en validación CEEMDAN de XLP")
        sys.exit(1)
