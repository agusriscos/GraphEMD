"""
Script para aplicar el analisis de componentes independientes (ICA) a los componentes IMFs de la ETF XLP (Consumer Staples Select Sector SPDR Fund) mediante CEEMDAN.

1. Carga los datos necesarios para la aplicacion del analisis de componentes independientes (ICA) a los componentes IMFs de la ETF XLP.
2. Calcula los parametros optimos de ICA (numero de componentes independientes, etc) XLP.
2. Aplica el analisis de componentes independientes (ICA) a los componentes IMFs de la ETF XLP mediante CEEMDAN con los parametros optimos calculados.
...
Lógica implementada
---------------------
- **Carga**: ``xlp_imfs_ceemdan.parquet`` (8 IMF oscilatorias + ``Residuo``; el residuo no entra en el ajuste ICA).
...
- **Calibración de** ``k``: para cada ``k ∈ {2,…,p−1}`` se ajusta ICA y se registran
  ``max|corr(Z_i,Z_j)|``, error de Frobenius de reconstrucción y ``R²`` medio. Si todas las ``k``
  alcanzan independencia práctica (``max|corr| < 0.01``), se adopta ``k=4`` (referencia MSCI /
  ``docs/20abr26``); si no, se elige el menor ``k`` con ``R²`` ≥ 90 % del máximo de la rejilla.
...
**Calibración:** todas las ``k=2…7`` logran ``max|corr(Z)| < 0.01`` → **k=4** (referencia MSCI).
En ``k=4``: error Frobenius rel. ≈ 0.69, ``R²`` medio columnas ≈ 0.61 (vs. ``k=7``: ``R²``≈0.93
pero casi rango completo).

**Salida aplicada:** ``Z_1…Z_4`` + ``Residuo`` en ``ica/fastica/imfs_reducidas.parquet``;
``xlp_ica_parametros.json`` con rejilla completa.

**Figura:** ``figures/xlp_componentes_ica_ceemdan.png`` (panel temporal de ``Z_j`` y residuo).
"""
...
def cargar_imfs_xlp(ruta_imfs: Path = _RUTA_IMFS_XLP) -> pd.DataFrame:
    ...
    ----------
    ruta_imfs : Path
        Ruta a ``xlp_imfs_ceemdan.parquet``.
    ...
    if not ruta_imfs.is_file():
        raise FileNotFoundError(
            f"No se encuentra {ruta_imfs}. Ejecute antes 03_ceemdan_xlp.py."
        )
    df = pd.read_parquet(ruta_imfs, engine="pyarrow")
    logger.info("IMFs XLP: %d filas, columnas %s", len(df), list(df.columns))
    return df
...
def calibrar_n_components_ica(
    ...
    k_referencia_msci : int
        ``k`` usado en MSCI / documento (solo referencia comparativa).

    Returns
    -------
    dict
        Bloques ``mejor``, ``referencia_msci``, ``rejilla`` y correlación nativa IMF.
    """
    p = int(X.shape[1])
    if p < 2:
    ...
    if (
        ...
        criterio_usado = (
            f"independencia práctica (max|corr|<{UMBRAL_CORR_Z_INDEPENDENCIA}) "
            f"en toda la rejilla → k={k_referencia_msci} (referencia MSCI)"
        )
    else:
        ...
        criterio_usado = (
            f"menor k con R² medio ≥ {FRACCION_R2_OBJETIVO:.0%} del máximo "
            f"({umbral_r2:.4f})"
        )
...
def cargar_indice_temporal_xlp(ruta_xlp: Path = _RUTA_XLP) -> pd.Index:
    """
    Carga el índice de fechas de ``xlp.parquet`` para el eje temporal.

    Parameters
    ----------
    ruta_xlp : Path
        Parquet de precios XLP.
    ...
    if not ruta_xlp.is_file():
        raise FileNotFoundError(f"No se encuentra {ruta_xlp}.")
...
def generar_panel_componentes_ica(
    ...
    fig.suptitle(
        f"XLP: componentes independientes FastICA ({titulo_k}) + residuo CEEMDAN",
        fontsize=11,
        y=1.002,
    )
    fig.tight_layout()
...
def generar_figura_desde_parquet(
    ...
    ruta_xlp : Path
        Precios XLP para el índice temporal.
    ruta_salida : Path
        PNG de salida.
    k : int, optional
...
def guardar_parametros_ica(
    ...
    aplicacion : dict
        Salida de :func:`aplicar_ica_xlp`.
    ruta_json : Path
...
def main(
    ...
    """
    Ejecuta calibración y aplicación de FastICA sobre las IMF XLP.
    ...
    logger.info("k seleccionado para ICA XLP: %d", k_optimo)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="FastICA sobre IMF CEEMDAN de XLP con calibración de k."
    )
    parser.add_argument(
        "--parquet-imfs",
        type=Path,
        default=None,
        help="Parquet IMF (por defecto xlp_imfs_ceemdan.parquet).",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Ruta del PNG (por defecto figures/xlp_componentes_ica_ceemdan.png).",
    )
    ...
    try:
        ...
        logger.exception("Error en ICA XLP")
        sys.exit(1)
