"""
Script para aplicar las transformaciones de grafos a los componentes IMFs de la ETF XLP (Consumer Staples Select Sector SPDR Fund) mediante CEEMDAN.

1. Carga los datos necesarios para la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLP. Por un lado los datos de los componentes IMFs obtenidos en la descomposición de la ETF XLP mediante CEEMDAN y por otro lado los datos de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLP.
2. Aplica las transformaciones de grafos a los componentes IMFs de la ETF XLP mediante CEEMDAN.
...
Lógica implementada
---------------------
- **Carga**: ``xlp_imfs_ceemdan.parquet`` (8 IMFs + ``Residuo``, 3587 observaciones; salida de
  ``03_ceemdan_xlp.py``).
...
- **Implementación**: ``obtener_grafos_all_imf`` guarda por componente parquet + ``.pt``
  (PyTorch Geometric) en ``data/GraphEMD/xlp_analysis/grafos/{hvg,nvg,recurrencia}/``.
- **Tabla auxiliar**: ``xlp_parametros_recurrencia_ceemdan.csv`` (``tau``, ``d``, ``epsilon`` por IMF).
...
**Recurrencia** (``tau``, ``d``, ``epsilon`` en ``xlp_parametros_recurrencia_ceemdan.csv``):
...
**Salidas:** ``grafos/{hvg,nvg,recurrencia}/``, ``xlp_resumen_grafos_imf.csv``,
...
def cargar_imfs_ceemdan_xlp(
    ...
    logger.info(
        "IMFs XLP cargadas: %d filas, componentes %s",
        len(df_imfs),
        columnas,
    )
    return df_imfs
...
def aplicar_transformaciones_grafos(
    ...
    """
    Genera HVG, NVG y grafos de recurrencia para cada IMF/residuo de XLP.
...
def main(
    ...
    """
    Ejecuta la pipeline de transformación IMF → grafos para XLP.

    Parameters
    ----------
    ruta_imfs : Path, optional
        Parquet de IMFs. Por defecto ``xlp_imfs_ceemdan.parquet``.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Transformaciones IMF→grafo (HVG, NVG, recurrencia) para XLP CEEMDAN."
    )
    parser.add_argument(
        "--parquet-imfs",
        type=Path,
        default=None,
        help="Parquet con IMF_1,...,Residuo (por defecto xlp_imfs_ceemdan.parquet).",
    )
    parser.add_argument(
        "--dir-grafos",
        type=Path,
    ...
    try:
        ...
        logger.exception("Error en transformaciones de grafos XLP")
        sys.exit(1)
