"""
Script para obtener las metricas de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLE.

1. Carga los datos necesarios para la obtencion de las metricas de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLE.
2. Obten las metricas de densidad, numero de enlaces, componentes conexas, diametro y grado medio de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLE.
3. Calcula los coeficientes de clustering (media, mediana y desviacion estandar) de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLE.
4. Calcula las centralidades (degree, betweenness, closeness y eigenvector) de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la ETF XLE.
...
Lógica implementada
---------------------
- **Carga**: grafos guardados por ``05_graph_transformations_xle.py`` en
  ``data/GraphEMD/xle_etf_analysis/grafos/{hvg,nvg,recurrencia}/{imf_k,residuo}/*.pt``
...
**Salidas:** ``xle_metricas_grafos_imf.csv``, ``xle_resumen_metricas_grafos.json``.
"""
...
_RUTA_METRICAS_CSV = _DIR_DATOS / "xle_metricas_grafos_imf.csv"
_RUTA_RESUMEN_JSON = _DIR_DATOS / "xle_resumen_metricas_grafos.json"

TIPOS_GRAFO: tuple[str, ...] = ("hvg", "nvg", "recurrencia")

# Umbrales alineados con scripts MSCI (exploracion/); clustering NVG acotado
# por coste en grafos densos (IMF_7+ en XLE superan ~4×10⁵ aristas).
...
def listar_componentes_grafos(dir_grafos: Path = _DIR_GRAFOS) -> list[str]:
    ...
    if not carpeta_hvg.is_dir():
        raise FileNotFoundError(
            f"No existe {carpeta_hvg}. Ejecute antes 05_graph_transformations_xle.py."
        )
...
def main(dir_grafos: Optional[Path] = None) -> dict[str, Any]:
    """
    Punto de entrada: calcula y guarda métricas de todos los grafos XLE.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Métricas estructurales, clustering y centralidad de grafos IMF XLE."
    )
    parser.add_argument(
        "--dir-grafos",
        type=Path,
    ...
    try:
        ...
        logger.exception("Error al calcular métricas de grafos XLE")
        sys.exit(1)
