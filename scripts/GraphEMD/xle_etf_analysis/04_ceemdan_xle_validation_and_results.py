"""
Script para validar y analizar los resultados de la descomposición de la ETF XLE (Energy Select Sector SPDR Fund) en componentes IMFs mediante CEEMDAN.

1. Carga los datos necesarios para la validacion y analisis de los resultados de la descomposición de la ETF XLE. Por un lado los datos de precio de cierre de la ETF XLE y por otro lado los datos de los componentes IMFs obtenidos en la descomposición de la ETF XLE mediante CEEMDAN.
2. Obten la información estadistica sobre energía, variancia, frecuencia y amplitud media de los componentes IMFs obtenidos.
3. Valida los resultados de la descomposición de la ETF XLE mediante CEEMDAN comparandolos con los resultados de la descomposición de la MSCI World en este mismo repositorio. Calcula el error de la reconstrucción de la serie original con los componentes IMFs obtenidos. Evalua como es el residuo de la descomposicion de la ETF XLE (mira si representa la tendencia a largo plazo de la serie original (monotona creciente o decreciente)).
4. Pinta las graficas de los componentes IMFs obtenidos y la serie original de la ETF XLE. Tambien pinta el residuo de la descomposicion CEEMDAN vs. EEMD para el XLE
...
Lógica implementada
-------------------
- **Carga**: ``xle.parquet``, ``xle_imfs_ceemdan.parquet``; referencia MSCI en
...
- **EEMD XLE**: parámetros por defecto MSCI (``info_msci_world_data``); figura
...
- **Figuras**: ``figures/xle_imfs_panel.png``, ``figures/xle_imf_decomposition_ceemdan_vs_eemd.png``,
  ``figures/xle_tendencia_compuesta_ceemdan.png``.

Resultados obtenidos (ejecución 2026-05-16, n=3587)
-------------------------------------------------

**Reconstrucción XLE:** ``rmse_relativo≈8.7×10⁻¹⁷`` (identidad numérica IMFs+residuo).

**IMFs XLE (patrón tipo MSCI):** IMF_1 — energía 156, 1282 ciclos, amp. 0.15;
IMF_8 — energía 27367, 2 ciclos, amp. 2.24; 8 IMFs en total.

**Residuo XLE:** media 28.36, rango [19.76, 48.28]; no estrictamente monótono
...
**Figuras:** ``figures/xle_imfs_panel.png``,
``figures/xle_imf_decomposition_ceemdan_vs_eemd.png`` (brecha CEEMDAN vs EEMD).

**Salidas:** ``xle_imf_metricas_ceemdan.csv``, ``xle_validacion_ceemdan.json``,
``xle_imfs_eemd.parquet`` (caché EEMD).
"""
...
_RUTA_IMFS_EEMD_XLE = _DIR_DATOS / "xle_imfs_eemd.parquet"
...
_RUTA_METRICAS_CSV = _DIR_DATOS / "xle_imf_metricas_ceemdan.csv"
_RUTA_VALIDACION_JSON = _DIR_DATOS / "xle_validacion_ceemdan.json"
_RUTA_FIGURA_TENDENCIA_COMPUESTA = (
    _DIR_FIGURAS / "xle_tendencia_compuesta_ceemdan.png"
)
...
def _cargar_modulo_info_msci() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("info_msci_world_data_xle", ruta)
...
def cargar_xle() -> tuple[pd.DataFrame, np.ndarray]:
    """
    Carga precios XLE y la serie de cierre.
    ...
    """
    if not _RUTA_XLE.is_file():
        raise FileNotFoundError(f"Ejecute 01_download_xle.py: falta {_RUTA_XLE}")
    df = pd.read_parquet(_RUTA_XLE, engine="pyarrow")
    serie = np.asarray(df["Close"].values, dtype=np.float64)
    return df, serie


def cargar_imfs_xle() -> pd.DataFrame:
    """
    ...
    """
    if not _RUTA_IMFS_XLE.is_file():
        raise FileNotFoundError(f"Ejecute 03_ceemdan_xle.py: falta {_RUTA_IMFS_XLE}")
    return pd.read_parquet(_RUTA_IMFS_XLE, engine="pyarrow")
...
def interpretacion_bibliografica_residuo_xle(
    validacion_reconstruccion: dict[str, Any],
    residuo_eval: dict[str, Any],
    tendencia_compuesta_eval: dict[str, Any],
) -> dict[str, Any]:
    """
    Redacta la interpretación del residuo XLE apoyada en literatura EMD/CEEMDAN.
    ...
    parrafo = (
        f"La descomposición CEEMDAN de XLE es matemáticamente consistente: la suma de "
...
def generar_figura_tendencia_compuesta(
    ...
    axes[0].plot(fechas, serie, color="0.35", linewidth=0.7, label="Close XLE")
    axes[0].plot(
        fechas,
        tendencia,
        color="C1",
    ...
    axes[0].set_title(
        f"XLE CEEMDAN: precio y tendencia compuesta (R² lineal={r2_comp:.3f})",
        fontsize=10,
    )
...
def comparar_con_msci(
    tabla_xle: pd.DataFrame,
    tabla_msci: Optional[pd.DataFrame],
    validacion_xle: dict[str, Any],
    validacion_msci: Optional[dict[str, Any]],
) -> dict[str, Any]:
    """
    Resume similitudes estructurales XLE vs MSCI World.
    ...
    """
    out: dict[str, Any] = {
        "n_imfs_xle": len(tabla_xle),
        "validacion_xle": validacion_xle,
    }
    if tabla_msci is not None:
        out["n_imfs_msci"] = len(tabla_msci)
        out["validacion_msci"] = validacion_msci
        out["frecuencia_imf1_xle"] = float(tabla_xle.loc["IMF_1", "frecuencia_ciclos"])
        out["frecuencia_imf1_msci"] = float(tabla_msci.loc["IMF_1", "frecuencia_ciclos"])
        out["amplitud_imf8_xle"] = float(
            tabla_xle.loc[tabla_xle.index[-1], "amplitud_media"]
        )
...
def obtener_imfs_eemd_xle(serie: np.ndarray, mod_info: Any) -> pd.DataFrame:
    """
    Obtiene IMFs EEMD de XLE (cache en parquet si existe).
    ...
    """
    if _RUTA_IMFS_EEMD_XLE.is_file():
        logger.info("EEMD XLE cargado desde caché: %s", _RUTA_IMFS_EEMD_XLE)
        return pd.read_parquet(_RUTA_IMFS_EEMD_XLE, engine="pyarrow")
    logger.info("Calculando EEMD para XLE (puede tardar varios minutos)...")
    ...
    df_eemd.to_parquet(_RUTA_IMFS_EEMD_XLE, engine="pyarrow", index=False)
    logger.info("EEMD XLE guardado en %s", _RUTA_IMFS_EEMD_XLE)
    return df_eemd
...
def generar_panel_imfs(
    ...
    axes[-1].set_xlabel("Fecha")
    fig.suptitle("XLE: precio de cierre e IMFs CEEMDAN", fontsize=11, y=1.002)
    fig.tight_layout()
    ruta_salida.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(ruta_salida, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def generar_figura_ceemdan_vs_eemd(
    ...
    """
    Figura estilo ``imf_decomposition.png`` (CEEMDAN vs EEMD) para XLE.
    ...
    destino = directorio / "xle_imf_decomposition_ceemdan_vs_eemd.png"
    if origen.is_file():
...
def main() -> dict[str, Any]:
    ...
    df_xle, serie_xle = cargar_xle()
    df_imfs_xle = cargar_imfs_xle()
    if len(df_xle) != len(df_imfs_xle):
        raise ValueError(
            f"Longitudes distintas: precios {len(df_xle)} vs IMFs {len(df_imfs_xle)}"
        )

    logger.info("=" * 70)
    logger.info("MÉTRICAS IMF — XLE (CEEMDAN)")
    logger.info("=" * 70)
    tabla_xle = tabla_metricas_imfs(df_imfs_xle)
    logger.info("\n%s", tabla_xle.to_string(float_format=lambda x: f"{x:.4f}"))

    validacion_xle = validar_reconstruccion(serie_xle, df_imfs_xle)
    residuo_xle = evaluar_residuo(serie_xle, df_imfs_xle)
    tendencia_compuesta_xle = evaluar_tendencia_compuesta(
        serie_xle, df_imfs_xle, residuo_xle
    )
    interpretacion_xle = interpretacion_bibliografica_residuo_xle(
        validacion_xle,
        residuo_xle,
        tendencia_compuesta_xle,
    )
    logger.info("Reconstrucción XLE: %s", validacion_xle)
    logger.info("Residuo XLE: %s", residuo_xle)
    logger.info("Tendencia compuesta XLE: %s", tendencia_compuesta_xle["expresion"])
    logger.info(
        "R² residuo=%.3f, R² compuesta=%.3f, corr(Close) residuo=%.3f, compuesta=%.3f",
        tendencia_compuesta_xle["metricas_residuo"]["r2_regresion_lineal"],
        tendencia_compuesta_xle["metricas_tendencia_compuesta"]["r2_regresion_lineal"],
        tendencia_compuesta_xle["metricas_residuo"]["correlacion_con_close"],
        tendencia_compuesta_xle["correlacion_tendencia_compuesta_con_close"],
    )
    ...
    comparacion = comparar_con_msci(
        tabla_xle, tabla_msci, validacion_xle, validacion_msci
    )

    _DIR_DATOS.mkdir(parents=True, exist_ok=True)
    tabla_xle.to_csv(_RUTA_METRICAS_CSV)
    payload = {
        "n_observaciones": len(serie_xle),
        "metricas_imf_xle": tabla_xle.reset_index().to_dict(orient="records"),
        "validacion_reconstruccion_xle": validacion_xle,
        "evaluacion_residuo_xle": residuo_xle,
        "evaluacion_tendencia_compuesta_xle": tendencia_compuesta_xle,
        "interpretacion_residuo_ceemdan_xle": interpretacion_xle,
    ...
    df_eemd = obtener_imfs_eemd_xle(serie_xle, mod_info)
    generar_panel_imfs(
        df_xle,
        df_imfs_xle,
        serie_xle,
        _DIR_FIGURAS / "xle_imfs_panel.png",
    )
    ...
    return {
        "tabla_xle": tabla_xle,
        "validacion_xle": validacion_xle,
        "residuo_xle": residuo_xle,
        "tendencia_compuesta_xle": tendencia_compuesta_xle,
        "interpretacion_xle": interpretacion_xle,
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error en validación CEEMDAN de XLE")
        sys.exit(1)
