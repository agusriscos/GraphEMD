"""
Script para aplicar el analisis de componentes independientes (ICA) a los componentes IMFs de la ETF XLE (Energy Select Sector SPDR Fund) mediante CEEMDAN.

1. Carga los datos necesarios para la aplicacion del analisis de componentes independientes (ICA) a los componentes IMFs de la ETF XLE.
2. Calcula los parametros optimos de ICA (numero de componentes independientes, etc) XLE.
2. Aplica el analisis de componentes independientes (ICA) a los componentes IMFs de la ETF XLE mediante CEEMDAN con los parametros optimos calculados.
...
Lógica implementada
---------------------
- **Carga**: ``xle_imfs_ceemdan.parquet`` (8 IMF oscilatorias + ``Residuo``; el residuo no entra en el ajuste ICA).
...
``xle_ica_parametros.json`` con rejilla completa.

**Figura:** ``figures/xle_componentes_ica_ceemdan.png`` (panel temporal de ``Z_j`` y residuo).
"""
...
def cargar_imfs_xle(ruta_imfs: Path = _RUTA_IMFS_XLE) -> pd.DataFrame:
    ...
    logger.info("IMFs XLE: %d filas, columnas %s", len(df), list(df.columns))
    return df
...
def cargar_indice_temporal_xle(ruta_xle: Path = _RUTA_XLE) -> pd.Index:
    """
    Carga el índice de fechas de ``xle.parquet`` para el eje temporal.

    Parameters
    ----------
    ruta_xle : Path
        Parquet de precios XLE.
    ...
    if not ruta_xle.is_file():
        raise FileNotFoundError(f"No se encuentra {ruta_xle}.")
...
def generar_panel_componentes_ica(
    ...
    fig.suptitle(
        f"XLE: componentes independientes FastICA ({titulo_k}) + residuo CEEMDAN",
        fontsize=11,
        y=1.002,
    )
    fig.tight_layout()
...
def generar_figura_desde_parquet(
    ...
    ruta_xle : Path
        Precios XLE para el índice temporal.
    ruta_salida : Path
        PNG de salida.
    k : int, optional
...
def guardar_parametros_ica(
    ...
    aplicacion : dict
        Salida de :func:`aplicar_ica_xle`.
    ruta_json : Path
...
def main(
    ...
    """
    Ejecuta calibración y aplicación de FastICA sobre las IMF XLE.
    ...
    logger.info("k seleccionado para ICA XLE: %d", k_optimo)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="FastICA sobre IMF CEEMDAN de XLE con calibración de k."
    )
    parser.add_argument(
        "--parquet-imfs",
        type=Path,
        default=None,
        help="Parquet IMF (por defecto xle_imfs_ceemdan.parquet).",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Ruta del PNG (por defecto figures/xle_componentes_ica_ceemdan.png).",
    )
    ...
    try:
        ...
        logger.exception("Error en ICA XLE")
        sys.exit(1)
