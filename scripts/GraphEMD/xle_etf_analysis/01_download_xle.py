"""
Script para descargar los datos de la ETF XLE (Energy Select Sector SPDR Fund) desde Yahoo Finance.

Lógica implementada
---------------------
- Descarga histórico de ``XLE`` con ``yfinance`` (``period="max"``).
- Recorte por fechas inclusivas ``2012-01-12`` … ``2026-04-20`` (misma ventana que ``docs/20abr26``).
- Si existe ``data/20abr26/msci_world.parquet``, se alinea el índice de XLE al de MSCI World
...
def _ruta_script() -> Path:
    ...
    -------
    Path
        Ruta al archivo ``download_xle.py``.
    """
...
def descargar_xle_yahoo() -> pd.DataFrame:
    """
    Descarga el histórico completo de XLE desde Yahoo Finance.
    ...
    """
    logger.info("Descargando %s desde Yahoo Finance...", SIMBOLO_XLE)
    ticker = yf.Ticker(SIMBOLO_XLE)
    df = ticker.history(period="max")
    if df is None or df.empty:
        raise ValueError(f"No se pudieron descargar datos para {SIMBOLO_XLE}.")
    logger.info(
...
def alinear_a_calendario_msci(
    ...
    """
    Reindexa XLE al calendario de fechas de ``msci_world.parquet``.

    Parameters
    ----------
    df_xle : pd.DataFrame
        XLE ya recortado a la ventana temporal.
    ruta_msci : Path
    ...
    -------
    pd.DataFrame
        XLE con el mismo índice que MSCI (solo fechas presentes en ambos).
    ...
    if faltantes > 0:
        logger.warning(
            "%s fechas MSCI sin cotización XLE; se eliminan esas filas.",
            faltantes,
        )
...
def main() -> dict[str, Any]:
    """
    Descarga XLE, alinea a la ventana MSCI, guarda parquet y calcula métricas.
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error en la descarga o el análisis de XLE")
        sys.exit(1)
