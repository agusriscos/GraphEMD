"""
Script para descomponer la XAU/USD (par spot XAU/USD (oro frente al dólar)) en componentes IMFs mediante CEEMDAN.

1. Carga los datos de la XAU/USD desde el archivo parquet data/GraphEMD/xauusd_analysis/xauusd.parquet.
...
3. Calcula los parametros optimos de CEEMDAN para el XAU/USD usando la misma logica que se ha realizado para el MSCI World en este mismo repositorio.
...
Lógica implementada
---------------------
- **Carga**: ``xauusd.parquet`` (ventana 2012-01-12–2026-04-20, 3587 observaciones alineadas al calendario MSCI).
- **Referencia MSCI** (``docs/20abr26``, tabla CEEMDAN): ``max_imf=14``, ``trials=100``,
  ``epsilon=0.05``, ``seed=42``.
- **Calibración XAU/USD** (rejilla ``epsilon`` × ``trials`` × ``max_imf`` con ``max_imf≥9``):
  en XAU/USD CEEMDAN suele detenerse en **8 IMFs** aunque ``max_imf`` sea mayor; el criterio
...
- **Descomposición final** con ``calibracion["mejor"]`` (criterio linealidad); salida en
  ``xauusd_imfs_ceemdan.parquet`` y ``xauusd_ceemdan_parametros.json``.

Resultados obtenidos
--------------------

Tras cada calibración, revisar ``xauusd_ceemdan_parametros.json`` (bloque ``mejor``) y
re-ejecutar ``04_ceemdan_xauusd_validation_and_results.py`` para figuras y validación.
"""
...
_RUTA_PARQUET_XAUUSD = _DIR_DATOS / "xauusd.parquet"
_RUTA_IMFS_SALIDA = _DIR_DATOS / "xauusd_imfs_ceemdan.parquet"
_RUTA_PARAMETROS_JSON = _DIR_DATOS / "xauusd_ceemdan_parametros.json"
...
def _cargar_modulo_emdsynth_pipeline():
    ...
    spec = importlib.util.spec_from_file_location("emdsynth_ejecutar_xauusd", ruta)
...
def cargar_serie_cierre_xauusd(ruta_parquet: Path = _RUTA_PARQUET_XAUUSD) -> tuple[pd.DataFrame, np.ndarray]:
    """
    Carga XAU/USD y devuelve el vector de precios de cierre.

    Parameters
    ----------
    ruta_parquet : Path
        Ruta a ``xauusd.parquet``.
    ...
    if not ruta_parquet.is_file():
        raise FileNotFoundError(
            f"No se encontró {ruta_parquet}. Ejecute antes 01_download_xauusd.py."
        )
...
def calibrar_parametros_ceemdan_xauusd(
    serie: np.ndarray,
    mod_emdsynth: Any,
    ...
    if mejor_lineal is None:
        raise RuntimeError(
            "Calibración CEEMDAN XAU/USD: ninguna configuración cumple el umbral de RMSE."
        )
    ...
    resumen_n_imfs = {
        ...
        "nota": (
            "CEEMDAN en XAU/USD alcanza como máximo 8 IMFs en esta ventana; "
...
def guardar_parametros_json(
    ...
    ----------
    calibracion : dict
        Salida de ``calibrar_parametros_ceemdan_xauusd``.
    resumen_imfs : dict
...
def main() -> dict[str, Any]:
    """
    Calibra CEEMDAN en XAU/USD, descompone con parámetros óptimos y guarda salidas.
    ...
    df, serie = cargar_serie_cierre_xauusd()

    logger.info("=" * 70)
    logger.info(
        "CALIBRACIÓN CEEMDAN — XAU/USD (rejilla %s×%s×%s, max_imf≥9, residuo lineal)",
        len(REJILLA_EPSILON),
        len(REJILLA_TRIALS),
        len(REJILLA_MAX_IMF),
    )
    logger.info("=" * 70)
    calibracion = calibrar_parametros_ceemdan_xauusd(serie, mod)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error en CEEMDAN de XAU/USD")
        sys.exit(1)
