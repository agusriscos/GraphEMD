"""
Script para analizar las condiciones de la XAU/USD (par spot XAU/USD (oro frente al dólar)) para su analisis con EMD (Empirical Mode Decomposition).
...
_RUTA_PARQUET_XAUUSD = _REPO_ROOT / "data" / "GraphEMD" / "xauusd_analysis" / "xauusd.parquet"
...
def cargar_datos_xauusd(ruta_parquet: Path = _RUTA_PARQUET_XAUUSD) -> pd.DataFrame:
    """
    Carga la serie XAU/USD y calcula retornos diarios en porcentaje.

    Parameters
    ----------
    ruta_parquet : Path
        Ruta al archivo ``xauusd.parquet``.
...
def main() -> dict[str, Any]:
    """
    Ejecuta el pipeline completo de condiciones EMD para XAU/USD.
    ...
    """
    df = cargar_datos_xauusd()

    logger.info("=" * 70)
    logger.info("TESTS DE ESTACIONARIDAD — XAU/USD")
    logger.info("=" * 70)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Error en el análisis de condiciones EMD de XAU/USD")
        sys.exit(1)
