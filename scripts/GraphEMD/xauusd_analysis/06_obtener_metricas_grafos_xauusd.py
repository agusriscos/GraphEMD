"""
Script para obtener las metricas de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la XAU/USD.

1. Carga los datos necesarios para la obtencion de las metricas de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la XAU/USD.
2. Obten las metricas de densidad, numero de enlaces, componentes conexas, diametro y grado medio de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la XAU/USD.
3. Calcula los coeficientes de clustering (media, mediana y desviacion estandar) de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la XAU/USD.
4. Calcula las centralidades (degree, betweenness, closeness y eigenvector) de los grafos obtenidos en la aplicacion de las transformaciones de grafos a los componentes IMFs de la XAU/USD.
...
Lógica implementada
---------------------
- **Carga**: grafos guardados por ``05_graph_transformations_xauusd.py`` en
...
**Salidas:** ``xauusd_metricas_grafos_imf.csv``, ``xauusd_resumen_metricas_grafos.json``.
"""
...
# por coste en grafos densos (IMF_7+ en XAU/USD superan ~4×10⁵ aristas).
...
def listar_componentes_grafos(dir_grafos: Path = _DIR_GRAFOS) -> list[str]:
    ...
    if not carpeta_hvg.is_dir():
        raise FileNotFoundError(
            f"No existe {carpeta_hvg}. Ejecute antes 05_graph_transformations_xauusd.py."
        )
...
def main(dir_grafos: Optional[Path] = None) -> dict[str, Any]:
    """
    Punto de entrada: calcula y guarda métricas de todos los grafos XAU/USD.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Métricas estructurales, clustering y centralidad de grafos IMF XAU/USD."
    )
    parser.add_argument(
        "--dir-grafos",
        type=Path,
    ...
    try:
        ...
        logger.exception("Error al calcular métricas de grafos XAU/USD")
        sys.exit(1)
